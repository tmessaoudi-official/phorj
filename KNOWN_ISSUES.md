...
0. **6C-hardened (2026-07-24, all fixed in the same slice):** the multipart PHP twin now mirrors
   ...
   target). Locked by `rich_request_multipart_agrees_on_all_legs` — 3-leg parity over the
   well-formed multipart path, the non-UTF-8 reject, AND a > 256 KiB SPILL round-trip (the first
...
- **SWEEP BATCH 1 (spine 7, code+repo hygiene)** — findings from the full review sweep:
  ...
  against NATIVE module names — an example importing only `Core.Database` (natives live in `Core.Native.Database`)
  ...
  (5) FIXED: FEATURES.md had NO Core.Database/Core.Mail rows (the flagship batteries absent from the
...
- **`Core.Database` typed-generic hydration (DEC-208 S2) — shipped + disclosures.** `List<T> = stmt.queryInto()`
  ...
  **Surface deviation (developer-authorized):** contextual inference, NOT the `<T>` turbofish DEC-208/
  ...
  type is `E-DB-INTO-NO-TYPE`. **Disclosures:** (1) under `import Core.Database`, `queryInto`/`queryOneInto` are
...
- **`Core.Database` transactions & correctness (DEC-208 slice C) — shipped subset + one PENDING adjudication.**
...
- **`Core.Database` writes & robustness (DEC-208 slice D) — shipped + disclosures.** Shipped
...
- **`Core.Database` typed hydration completion (DEC-208 slice B) — shipped + disclosures.** Three shape-directed
  ...
  method names under `import Core.Database`. (b) A self-referential row class (`class Employee { …, public
...
- **`Core.Database` value mapping (DEC-208 slice E) — shipped subset (enum/decimal/JSON) + `DateTime` deferred.**
...
- **`Core.Database` compile-time safety (DEC-208 slice F) — SQL-injection lint shipped; arity check DEFERRED.**
  **Shipped:** the `W-SQL-INJECTION` compile-time lint (`src/checker/calls/methods.rs`,
  ...
  the receiver types to the `Core.Database` `Connection` class AND the program imports `Core.Database` (module or member
  ...
  non-interpolated literal never warn; a coincidental user class named `Connection` with a `prepare` method never
  warns (no `Core.Database` import). It is a WARNING (rides the warning channel, never fails the build) so a
  ...
  - **DEFERRED — the placeholder/bind arity check (`E-BIND-ARITY`).** Not shipped: a sound compile-time
    ...
    (slice A) foundation makes a sound check cheap. See `docs/archive/specs/2026-07-14-core-db.md` slice F.

- **`Core.Database` multi-driver + Postgres (DEC-208 slice I) + credential Secret (slice G) — shipped subset +
  disclosures.** `new Connection(dsn)` dispatches on the DSN scheme behind a `DriverConn` trait
  ...
  - **Disclosures / boundaries:** (a) **No oracle.** There is no clean pure-Rust *synchronous* Postgres
    driver to differential the PHP-PDO leg against, so Postgres (like all of `Core.Database`) is
    ...
    it (one impl + one dep admission). See `docs/archive/specs/2026-07-14-core-db.md` slices G/I/J/K.
...
## §SERVE-CONFIG-PROVENANCE — a `ServeConfig` field written AT its class default reads as unset (P2, 2026-08-23) — **FIXED 2026-09-05 (DEC-475)**

**Was:** S3.2 Part C made the registered `Http.ServeConfig` the default source for what `phg serve`
binds, with a passed flag overriding it loudly — but it could not tell which fields the program had
actually SET. On the flag side provenance is observable (`Option::is_some`); on the config side
`new ServeConfig()` filled `port` with `8080`, indistinguishable from a program that wrote
`port: 8080` by hand. `serve::settings` therefore approximated "set" as *differs from the declared
class default*, with two accepted consequences: `new ServeConfig(timeout: 0)` could not express "no
timeout" (`0` WAS the class default, so it read as unset and the CLI's 30s default applied — the one
thing the field exists for), and a field written at its default got no override notice. A negative
value fell back to the default as a deliberate stopgap, because refusing it would have meant
refusing the class default too.

**Fixed by DEC-475:** every `ServeConfig` field is nullable and defaults to `null`, so `null` means
unset and every other value was stated on purpose. The effective defaults moved to where each value
is consumed (`DEFAULT_HOST` / `DEFAULT_PORT` / `DEFAULT_TIMEOUT_SECS` / `DEFAULT_TLS_MIN_VERSION`,
pinned against the prelude's documented defaults by
`the_effective_defaults_match_what_the_prelude_documents` so the two cannot drift). `timeout: 0` now
means no timeout; an explicitly-written default is observably set and its override IS announced.
Range validation landed with it as `E-SERVE-CONFIG-RANGE` — `port` 1..=65535, `workers` >= 0,
`timeout` >= 0, `maxBodySize` >= 1 — checked before the socket binds, which the value-comparison
rule could never have done.

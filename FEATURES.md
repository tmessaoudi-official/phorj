...
| Member-error namespacing: injected module members are qualifiable everywhere — `catch (DatabaseModule.Timeout e)`, `throws Mail.TlsError`, `UriModule.UriError` annotations, `throw new UriModule.UriMalformed(…)`; bare member-imported names stay the alias | ✅ | DEC-234; routed through the UA-L2 `module_of` registry |
...
| `Core.DatabaseModule`: multi-driver SQL (bundled SQLite default; Postgres `db-postgres`; MySQL/MariaDB `db-mysql`) — prepared statements, typed rows, `queryInto<T>`/`queryScalar`/`queryMap` hydration, lazy `streamInto<T>`, transactions + savepoints + retry, typed `DatabaseError` taxonomy, `Secret` credentials, `W-SQL-INJECTION` lint | ✅ | native-only (`E-TRANSPILE-DB`, §14 LADDER); gated by `tests/db.rs` on both backends |

...
| `Core.Debug`: `dump(x)` → `Dumped<T>` (pass-through `.value()` + captured `.text()`, deep deterministic rendering) · `dd(x)` dump-and-exit-1 · `Runtime.exit(code)` clean termination | ✅ | DEC-238; transpile gated until the PHP twin renderer lands |

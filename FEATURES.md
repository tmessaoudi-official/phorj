...
| Member-error namespacing: injected module members are qualifiable everywhere — `catch (Db.Timeout e)`, `throws Mail.TlsError`, `Uri.UriError` annotations, `throw new Uri.UriMalformed(…)`; bare member-imported names stay the alias | ✅ | DEC-234; routed through the UA-L2 `module_of` registry |
| Extension methods = UFCS (ratified): any in-scope free function whose first parameter matches the receiver's type is callable in method position — scalars, classes, extra args, chains; a real method wins over a same-name extension; statically checked + rewritten pre-backends; import-gated (nothing-in-the-wind) | ✅ | DEC-244 (2026-07-16 desk ruling: no new syntax — UFCS IS the story; PHP 8.6 still drafts scalar extensions); `examples/guide/extension-methods.phg` |
...
| `Core.Debug`: `dump(x)` → `Dumped<T>` (pass-through `.value()` + captured `.text()`, deep deterministic rendering) · `dd(x)` dump-and-exit-1 · `Runtime.exit(code)` clean termination | ✅ | DEC-238; three-backend byte-identical (conformance golden `lang/dump.phg`); erased-shape PHP-leg disclosure in KNOWN_ISSUES |

...
| Extension architecture (DEC-273): minimal core + flag-gated Rust extensions; `phg extensions` listing; `E-EXTENSION-DISABLED` names the flag; `docs/EXTENSIONS.md` generated | ✅ (waves 1-2) | registry + 12 extensions migrated to `src/ext/` (ini/crypto/regex/csv/encoding + json/uri/path/hash/decimal/test/debug incl. their preludes); db/mail/http-client/session/html/di move with wave 3 |

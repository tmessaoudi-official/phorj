...
| Extension architecture (DEC-273): minimal core + flag-gated Rust extensions; `phg extensions` listing; `E-EXTENSION-DISABLED` names the flag; `docs/EXTENSIONS.md` generated | ✅ (wave 1) | registry + 5 extensions migrated (`ini`/`crypto`/`regex`/`csv`/`encoding` in `src/ext/`); remaining move with later waves |

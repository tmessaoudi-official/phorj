...
**Hook tally (top-level scripts): 1 strong IN + 4 optional IN (gate set + helper) + 18 OUT = 23.** ✅ *(⚠ CORRECTED: `hooks/` holds **39** files in total — the extra 16 are the `session-remember/` subtree's scripts, prompts and tests, all covered by that subsystem's single OUT verdict below.)*
...
already-installed md files = **199 files** ✅ every file has an explicit verdict. *(⚠ CORRECTED per-category counts: skills 48, hooks 39, refs 4, bin 34, mcp 48, project-template 3 = 176 under `global/`, plus the 10 remaining `global/` root files = 186, plus 13 files outside `global/` (bundle README, MANIFEST, `claude-import.sh`, `lib/`) = **199**. The grand total was always right; three per-category figures were not.)*

...
| `mcp/**` (57 files: desktop-automation MCP server w/ X11/Wayland/Windows drivers, Jira/Confluence/GitLab/Trivy client configs + `.env` files + 2 design/plan docs + uv.locks) | **HARD OUT — security rationale** | (a) zero relevance to a language compiler; (b) contains corporate-tooling artifacts (`jira.env`, `confluence.env`, `gitlab.env`, scrubbed `<mcp-client-N>` placeholders). **`tmessaoudi-official/phorj` is a public GitHub repo** — committing corporate MCP configs, even scrubbed, is an information-exposure risk with no upside. Do not import under any option |

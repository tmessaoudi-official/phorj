...
at the time of the move — the baseline held zero frozen violations for any of the six moved in that pass (the ruled-build-order file was archived later, in the same series), so no
...
| `2026-07-26-ruled-build-order.md` *(folded 2026-09-02)* | **FOLDED into `docs/plans/MASTER-PLAN.md` §0.06**, because while it lived outside MASTER-PLAN it was the de-facto queue — the parallel-SSOT shape Invariant 19 forbids. Its header claimed "every item below is RULED and NOT YET BUILT … Wave 5.5 is the only row BUILT", false since 2026-07-29; the folded section corrects it and marks Wave 0, Wave 1 and Wave 3's DEC-364/347/348 as built |
| `SLICE-STATE-ARCHIVE.md` *(created 2026-09-02)* | **Not a plan — the tail of `docs/plans/SLICE-STATE.md`**, which was cut from 4,320 lines to the live cursor plus the two most recent superseded ones. This holds the ~two dozen older dated cursor, session and handoff blocks, several of which still call themselves "CURRENT CURSOR" (four did so simultaneously, which is the confusion the cut fixed). Content preservation was verified by set AND multiset comparison, not by line arithmetic. The Json-ADT JIT build plan that sat among these was NOT archived — it is live design, promoted to `docs/plans/json-adt-jit.plan.md` |

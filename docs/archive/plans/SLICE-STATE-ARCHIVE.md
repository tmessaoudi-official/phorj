# SLICE-STATE archive — superseded cursors and session logs

> Moved out of `docs/plans/SLICE-STATE.md` on 2026-09-02 by the post-Slice-3 consolidation. **Nothing
> was deleted** — `git grep` still finds every line of it here. SLICE-STATE kept the live cursor plus
> the two most recent superseded ones; everything below is older than that.
>
> **Do not read this as current.** It contains roughly two dozen dated cursor, session and handoff
> blocks, several of which still call themselves "CURRENT CURSOR" — four did so simultaneously, which
> is the specific confusion this move fixes. The live cursor is the top of `SLICE-STATE.md`; the
> roadmap is `MASTER-PLAN.md`; rulings are the decision register.
>
> One block did NOT come here: the Json-ADT JIT build plan, which is live design for the next perf
> slice and was promoted to `docs/plans/json-adt-jit.plan.md`.

...
| `regex` (BurntSushi) 1.x | ReDoS-safe regex | `Core.Regex` (`Regex.compile`) | `regex` | RE2-style finite automaton, guaranteed linear-time, exhaustively fuzzed; its restricted feature set (no backref/lookaround) is exactly the regular subset PHP `preg_*` matches identically, so the byte-identity spine holds; PCRE-only syntax is rejected at CHECK time for a literal pattern (`E-REGEX-UNSUPPORTED`) and at run time for a dynamic one, on every leg; syntax the crate and PCRE read differently (class-set operators, POSIX classes, `\v`, …) is rejected on BOTH constructors. ReDoS-immunity is a native-leg property — the PHP leg inherits PCRE, and a catastrophic regular pattern is a loud, named PHP-leg fault (KNOWN_ISSUES §Core.Regex) |
| `fancy-regex` 0.19 | opt-in backtracking regex (DEC-461, 2026-09-02; 0.11 → 0.19 on 2026-09-13) | `Core.Regex` (`Regex.compileBacktracking`) | `regex` | PCRE-class syntax (look-around, back-references, atomic groups, possessive quantifiers) on a backtracking VM that delegates the regular subset to `regex`; a STEP BUDGET (`backtrack_limit`, 1 000 000 — PHP's default `pcre.backtrack_limit`) raises a typed fault instead of hanging for the patterns the backtracking VM runs (those using PCRE-class syntax), and the PHP twin maps `PREG_BACKTRACK_LIMIT_ERROR` to the same fault; a regular-subset pattern is delegated to `regex` (linear natively) and its PCRE limit hit is a separate, named PHP-leg fault |
...
| `rustls` 0.23.x | TLS (6th domain, admitted 2026-07-03) | `Core.HttpClient` https (outbound) **and `phg serve` inbound TLS — BUILT in S3.5, DEC-331 D7** | `http-client` **and `http-server-tls`** (both non-default) | Memory-safe TLS; admitted to gate the HTTP client, and the SAME crate at the SAME feature set now terminates inbound TLS. **S3.5 added NO crate and NO rustls feature**: 0.23 has no client/server split, so `ServerConfig`/`ServerConnection`/`StreamOwned` were already compiled in. PEM decoding is hand-rolled in `src/serve/pem.rs` rather than admitting a 15th crate for it. Native-only, spine-quarantined |

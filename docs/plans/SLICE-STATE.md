...
> **DONE (2026-09-24): row 5l** (DEC-532, Q-0908-1) — `throw e` is a `never`-typed expression right of `??`, in an
> `if`-expression arm, a `match` arm or a lambda body (`E-THROW-POSITION` elsewhere); all legs, lifter, LSP, editors.
...
> **DONE (2026-09-25): row 5t** (`TBD`) — DEC-537: a direct assignment in a narrowed block checks against the declared type and the narrowing follows the value (checker + VM `stmt_narrowed`); nested assignments keep the narrowed check (scope ruling 20:01).
> **THEN:** row 5s (DEC-536, tuple field assignment through a `mutable` place), then L3.
...
> so its loss is NOT JIT coverage — what it IS was not examined here). Pinned by 4 tests in `src/jit/tests/decline_reasons.rs`. **DEC-520 (INT-ONLY inner lists) is BUILT, 2026-09-09** (`8e814277` M-Decomp + `017df010` the kind): `Kind::IntListList(Own)` across 12 non-test code sites in 4 files, the inner read reusing `IntList`'s flat encoding exactly as ruled. Two estimate corrections are recorded in the register: the `compile/mod.rs` ABI gate needed NO edit (it is an allow-list), and the two zero-headroom emit files forced an M-Decomp split as a separate behaviour-neutral first commit. The 4 decline pins FLIPPED to compile + `hits > 0` + oracle identity, and a 5th pins the kept boundary (a third nesting level and `List<List<string>>` still decline at `MakeList`). **L4c CLOSES; L3 is still blocked behind L4b**, which is now the live lane. Perf before/after is OWED under DEC-507/Invariant 11 — the box read load 20.07 during the build, so no number is claimed. **L4b**

...
> **NOW (2026-09-14): row 5h0** — the interpolation span collision found at 5h's 3C (KNOWN_ISSUES
> §interpolation-spans), fixed first as its own `fix:` commit because DEC-527's file-window check is unsound
> without it. **THEN:** build 5h, same-package half first — `try_ufcs` step (1) looks up

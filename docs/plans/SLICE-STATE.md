...
leverage order — **charset BUILT 2026-09-04** (DEC-494/495 — both legs hand-rolled from one table, no crate, no ini extension); next = `String.foldAccents` (step 6b) then `Time.sleep` + DEC-204; (3) DEC-333 perf. **Register rows: DEC-460 … DEC-495** (`C-decisions.md`, the

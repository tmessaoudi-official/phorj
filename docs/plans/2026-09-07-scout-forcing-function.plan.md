...
- **Two printers, two `escape_str`s (found by row 4b, 2026-09-26).** `src/lift/printer/exprs.rs` escapes any control
  character in a string literal as `\u{HEX}` (row 5n); `src/format/printer/atoms.rs` does not, so `phg format` can print a raw
  control byte where the lifter would not. Row 4b shared `escape_interp` only; unifying `escape_str` (and ideally `str_lit`)

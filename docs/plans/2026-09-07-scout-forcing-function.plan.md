...
| 5w | DEC-539 — a PHP static local (`static $x = …;`) gets a NAMED refusal instead of `` `static` is not supported in Tier-1``, naming the two rewrites (a private static field, or dropping the memo when the value is pure). Scout's one site, `TenureClassifier.php:1497`, is hand-ported in L3 | S | todo | - | src/lift/parser/* |

...
| 5j | NEW-C — a PHP `readonly` non-promoted property assigned once in the ctor body (`TenureSignal::$length`) lifts to `E-ASSIGN-IMMUTABLE`. DEC-524 RULED 2026-09-13: the ctor-once rule — an immutable field with no initializer may be assigned in its own class's constructor body, anywhere else stays `E-ASSIGN-IMMUTABLE`; checker + transpile + LSP + editors + example; the definite-assignment shape is surfaced at the build's 3C. Gates L3 | L | todo | - | src/checker/* src/transpile/* src/lsp/* editors/* examples/* src/lift/* |
...
| 5o | §LSP-CLASS-QUALIFIED-MEMBERS — hover, go-to-definition and completion on `Class.member` answer nothing for every member kind (pre-existing, Invariant 17). Resolve a class-name receiver to its constants, statics and static methods in all three requests; tests per member kind. Queued ahead of 5j | M | todo | - | src/lsp/* |
| 5p | P0 found at the DEC-534 research (2026-09-25): `Map.merge`'s PHP leg is `array_merge`, which RENUMBERS int and numeric-string keys — an int-keyed merge prints `5=a 9=y 7=b` on the VM and `0=a 1=x 2=b 3=y` transpiled (Invariant 1). Fix: transpile to `array_replace` (keys kept, later value wins, first position kept — exactly the kernel's rule); red differential test first | S | todo | - | src/native/map.rs tests/differential.rs |
| 5q | DEC-534 — `Map.union(a, b)`, left-biased like PHP `+`, transpiles to `$a + $b`; all legs, LSP, example, bench; the lifter maps PHP `+` where both operands are known arrays. Unblocks scout `Core/Text.php:137` | M | todo | - | src/native/map.rs src/lift/* tests/differential.rs examples/* bench/micro/* |

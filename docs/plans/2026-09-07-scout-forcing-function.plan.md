...
| 5n | §LIFT-UNICODE-ESCAPE — the lifter copies PHP double-quoted `\u{…}` as literal text (silent meaning change; Text.php's APOSTROPHES). Decode it to the character, audit `\x`, octal and `\$` on the same path; red tests + a round-trip case. Ruled next | M | todo | - | src/lift/* tests/lift_roundtrip.rs |

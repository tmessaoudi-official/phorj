...
| 5m | DEC-533 — collection constants: a class `const` holding a List/Map/Set literal of literal constants, explicit type, built once; checker + VM + tree-walker + transpile (PHP `const array`) + lift (name kept, type inferred from a homogeneous literal or an `@var` docblock, mixed literal refused by name; the false docblock hint fixed) + LSP + both editors + example. 41 of scout's 54 `const array` sites lift 1:1 as ruled (7 nested, 5 constant-referencing, 1 `string`/`null` — widening is a plan-gate question). Gates L3 (Text.php); Text.php:137 map union is the next wall, not in scope | L | todo | - | src/checker/* src/compiler/* src/interpreter/* src/transpile/* src/lift/* src/lsp/* editors/* examples/* |

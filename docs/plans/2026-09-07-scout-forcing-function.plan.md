...
| 5v | L3 wall — `foreach ($xs as [$a, $b])` (positional destructure in a `foreach`, DEC-510's form) has no lift: `TenureClassifier.php:1147` (once 5u + the by-ref hand-port are in), `:1650`, `:1789`, `Amenities.php:147/184`, `Heating.php:130`, `DetailHydrator.php:266` (with a skipped slot `[, $i, $l]`). phorj has `for ((a, b) in xs)` (`examples/guide/tuples.phg:45`), so this is mechanical under DEC-510; the checker arbitrates that elements are tuples | M | todo | - | src/lift/* tests/lift_roundtrip.rs |

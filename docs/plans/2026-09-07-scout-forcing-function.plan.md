...
| 5u | DEC-538 — PHP spread lifts to `List.concat`/`List.flatten` when every operand is a known list; a map or unknown-typed spread is refused by name. L3's first wall once `TenureClassifier.php`'s by-ref closure is hand-ported (`:365` `array_merge(...array_values($signals))`, `:367` `[...$evidence, ...$doubts]`); also `CriteriaEngine.php:378`, `HtmlSource.php:397` | M | todo | - | src/lift/* tests/lift_roundtrip.rs examples/lift/* |

...
| 5k | DEC-531 — keyword-named PHP names. (1) Parser: a keyword is accepted as a member name after `function` in a class body, as a field name, and after `.`/`?.`; LSP (completion, hover, go-to-definition, find-usages, symbols) and both editor grammars stop coloring `.match` as a keyword; transpile emits the name unchanged. (2) Lifter: a keyword-named local or parameter gets a `_` suffix scoped to its function, collision-free; a PHP named argument is rewritten or refused. Scout: `match` ×5 + `open` ×4 methods (25 call sites, 9 files), `type` field ×3 (10 accesses), locals `$type` ×44, `$match` ×5, `$new` ×4, `$class` ×3. Gates L3 (`Rent.Core` checks) | L | todo | - | src/parser/* src/lift/* src/lsp/* editors/* src/transpile/* tests/* examples/* |
...
### Known issues
- **Lift wall found by row 5e (2026-09-24): a PHP method named after a phorj keyword** — RULED DEC-531, build queued as row 5k. scout's

...
- [2026-09-25 19:02] AGREED: **A named tuple's field is assignable through a `mutable` place (DEC-536, Q-0925-2).**
  ...
  (corrected at build: POSITIONAL per DEC-504 — `$kept[0][1] = …`).
...
- [2026-09-26 06:27] AGREED: **PHP spread lifts to `List.concat`/`List.flatten`, no new syntax (DEC-538, Q-0908-2).**
  Only when every operand is a KNOWN list; a map or unknown-typed spread is refused by name. Build = row 5u, ahead of
  L3 (the classifier's first wall after the by-ref hand-port). REJECTED: list-literal spread syntax; refuse-by-name only.
- [2026-09-26 08:05] AGREED: **DEC-538 refined — the CHECKER arbitrates "known list".** Measured: a lifter-side
  declared-only test would leave `TenureClassifier.php:365/367` refused (`$signals` is an undeclared int-keyed literal,
  `$doubts = []` undeclared). The lifter maps every spread whose operands are not PROVABLY maps (keyed-string literal,
  known map constant, Map-declared var → refused by name); a non-list operand fails `phg check` on `List.concat` /
  `List.flatten`. Sound because a phorj List is sequential, so PHP's renumbering spread agrees whenever the draft checks.
  REJECTED: a lifter declared-list registry (strict).
...
> **Follow-up, row 5u (2026-09-26):** the `found Dot` class is gone: of those spread sites `CriteriaEngine.php` now
> lifts, `HtmlSource.php` moves on to line 607, and the two `f(...)` first-class callables
> (`Core/Redact.php`, `Rent/Core/PlafondBands.php`) carry a named Tier-2 refusal. Scout 82 → 83 of 155.
...
| 4a | L3a — WALL CENSUS of the 11-file classifier cluster (ruling 16:09): scratch copy, one bypass per commit until the cluster lifts (13 bypasses, 0 refused), then `phg check` (224 distinct errors — checking the entry package also checks the imported `Scout\Core`, so a second run double-counts; 6C-corrected from 270), classified. Docs only — § "L3a — wall census" | M | done | fb9f76b0 | docs/plans/2026-09-07-scout-forcing-function.plan.md |
| 4b | L3a/B13 — LIFTER P0: a PHP ternary inside a concatenation lifts to an if-expression INSIDE an interpolation whose nested braces/strings the phorj lexer rejects (`RawListing::text()`: `unexpected '}' in string`) — a draft that does not lex. BUILT 2026-09-26: root cause is PRINTER DRIFT, not the lexer — the lexer closes a hole at the first unescaped `}` by design, and `phg format`'s `str_lit` already escapes a hole's printed form (`escape_interp`: `\\`, `"`, `}`); the lift printer never did. `escape_interp` widened to `pub(crate)` and shared, so a lifted hole is byte-for-byte what `phg format` prints (the format pass `cmd_lift` runs cannot undo it). Probed first on all three legs, incl. a nested hole inside a nested string. 5 lifter tests (`lifter_tests_interp_holes.rs`: top-level ternary, non-string arms, scout `NtfyChannel`'s nested shape, a `match`, the format fixed point) + the `concat_conditional_holes` round-trip, all red first on the lex error; two `lifter_tests.rs` assertions re-pinned to the escaped (canonical) hole text. Sabotage (escape `}` only, not `"`) reds the nested and fixed-point tests; restore sha256-identical. Scout: lex-failing drafts **6 → 0 of 91**; the census copy with B13 reverted lifts `RawListing.phg` with the `{if (` hole and it lexes. Found, not fixed: the two printers' `escape_str` have drifted too (only the lift one escapes control chars, row 5n) — Known issues | S | done | 402ff8a3 | src/lift/printer/exprs.rs src/format/printer/atoms.rs src/lift/lifter_tests_interp_holes.rs src/lift/lifter_tests.rs tests/lift_roundtrip.rs |
...
| 4h | L3a check — 16 × `an empty collection needs its type` + 2 × `cannot infer a type from null` on `$x = []` / `$x = null` locals the lifter did not type (cause NOT root-caused; likely the DEC-515 seed ceiling) | M | todo | - | src/lift/* |
...
| 5s | DEC-536 — a named tuple's field is assignable through a `mutable` place (`kept[0].tags = v` rebuilds the tuple; Swift struct semantics). Scout `Dedup.php`'s per-key write. BUILT 2026-09-25: no backend change — the checker records each target `.f` step's position (as for a read), so DEC-504's `rewrite_tuple_fields` (which already visits targets) hands the VM `SetPathLocal`, the tree-walker `set_nested` and PHP `$kept[0][1] = …` (positional — the ruling text's `['tags']` was wrong). A value place = `mutable` local + `[i]`/tuple-`.f` steps; a `.f` step counts only once `check_expr` has recorded it, so a class-field step under a local root (`b.row.id`) is refused in the checker rather than reaching the compiler's `unreachable!` (3C finding); `t.tags[0]` works because a place with a `.f` step is type-checked before its root is resolved, keeping every existing diagnostic unchanged. `+=` works (desugars). `phg explain E-ASSIGN-TARGET` rewritten (it still said field/element assignment were a later slice). 10 checker tests (7 red first; 3 guards incl. `b.row.id`/`f().id`), a three-leg differential incl. no-aliasing, a `Dedup`-shaped lift round-trip, an LSP hover+diagnostics test, `examples/guide/tuple-field-assign.phg`; sabotages red. Perf OWED: the Dedup shape appends to a list held inside a tuple held in a list — COW copies on each append (not measured under load ~20). JIT declines the example's `group` on `MakeRange` (unrelated) — correct, not fast. Class-field/call-rooted places stay refused (KNOWN_ISSUES §TUPLE-FIELD-WRITE) | M | done | c0d36852 | src/checker/* tests/differential.rs tests/lift_roundtrip.rs examples/guide/tuple-field-assign.phg |
| 5t | DEC-537 — an assignment inside a narrowed block is checked against the DECLARED type, and the narrowing follows the assigned value's type from there; the VM's slot-keyed primitive narrowing updated at the assignment (Invariant 7). Scope ruled 20:01: DIRECT statements only. BUILT 2026-09-25: `Binding.declared` rides on each narrowing shadow (a guard shadow REPLACES the authored binding in the same scope map, so nothing below holds the declared type — 3C finding); `check_block_narrowed` runs the branch in its shadows' own scope, so "direct" = innermost + no other live narrowing of the name; a nested assignment keeps the narrowed check with a DEC-537 hint. VM: `compile_if` compiles direct statements through `stmt_narrowed`, which retypes an assigned narrowed slot to the value's `CTy` — the differential was red on the VM alone first (`expected int, found float` on `x = 2.5; x * 2.0`). 10 checker tests (2 guards green before) + a 4-function three-leg differential + a lift round-trip of PHP's first-match idiom + `examples/guide/narrowing-reassign.phg`; 7 sabotages red. 6C (advisor) asked whether the VM misses a primitive GUARD-TAIL narrowing: it cannot — the checker narrows no union-to-primitive guard tail either (DEC-184's lockstep bound; general fix W2-12) (`if (!(x is int)) { return 0; } x + 1` is refused on every leg [Verified 2026-09-25]), so the VM retypes at if-branch statements only. No grammar change; LSP shares the check pipeline | M | done | fb6cd7c2 | src/checker/* src/compiler/* tests/differential.rs examples/guide/narrowing-reassign.phg |
| 5u | DEC-538 — PHP spread lifts to `List.concat`/`List.flatten`; the CHECKER arbitrates list-ness (refinement 08:05) and only a provably-map operand is refused by name. BUILT 2026-09-26: lexer `Ellipsis` (greedy, as PHP), `PhpExpr::Spread` produced only as an unkeyed array element or a call argument; `lifter/spread.rs`: a list literal with a spread → `List.concat(a, b)` for two segments, else `List.flatten([…])` (a lone `[...$a]` is `flatten([a])` so the checker still sees the operand); `array_merge(...X)` → `List.flatten(X)`; an `array_values($m)` operand → `Map.values(m)` (phorj `Map` keeps PHP's insertion order, overwrites in place — MEASURED on all legs before the build, 3C finding); the statement `array_unshift($v, ...X)` → `v = List.concat(X, v)`. Refused BY NAME: a spread in a keyed literal, a provably-map operand (keyed literal, known map const, Map-declared var — new `MAP_VARS` beside the shape maps, closure-snapshotted), a keyed spread element, and every unconsumed spread (`lift_expr`'s `Spread` arm, DEC-299 pointer) — so `new P(...$xs)`, `max(...$xs)`, `sprintf(…, ...$xs)` all name it. Same token: a first-class callable `f(...)` and a variadic parameter now carry named refusals instead of `found Dot`. 12 lifter tests + the `spread_over_lists` round-trip (incl. the overwrite-order map), red first for `found Dot`; seven sabotages red (S7's first form did not compile — re-run as S7b). Example `examples/lift/spread` (3 legs = the original PHP). Scout 82 → 83/155 (`CriteriaEngine.php` lifts; `HtmlSource.php` → line 607; `Redact.php`/`PlafondBands.php` → named first-class-callable refusal). With the by-ref closure hand-ported in a SCRATCH copy, `TenureClassifier.php` passes both spreads and stops at line 1144 `foreach (… as [$a, $b, $c])` (row 5v). Not certified: a Map-declared FOREACH BINDER shadowing a list variable is a conservative by-name refusal (loud); a keyed literal inside a spread segment of a `@var list<array{…}>` local stays a Map (loud at check). 6C (advisor): the refinement's middle path had no test — a call-result map spread (lone, in a pair) and a list given to `Map.values` now each LIFT and fail `phg check` on the native's own signature; loosening `List.flatten`'s signature reds it (follow-up `78a837ac`). Disclosed, pre-existing and loud: the long form `array(…)` is not lifted at all, and a plain `array_values($list)` lifts to `List.enumerate` (a Map) — KNOWN_ISSUES §LIFT-SPREAD. No LSP/editor surface (lifter only) | M | done | d57b413e | src/lift/* tests/lift_roundtrip.rs examples/lift/spread.* |
| 5v | L3 wall — `foreach ($xs as [$a, $b])` (DEC-510's positional destructure in a loop header). BUILT 2026-09-26: `PhpStmt::Foreach.value` became `PhpForeachValue { Var, Tuple }` so all four binder consumers are compiler-forced (hoist binds each, the keyword walk renames each, the lift declares each); the parser reuses `destructure_binders` (keyed/nested refusals share DEC-510's words) after a skipped-slot scan (`[, $x]` — `_` is not a legal binder, MEASURED `E-NAME-CASE`), and refuses a single binder and a key with a destructured value by name. The lift emits phorj's own tuple-loop lowering (`for (var __fortup_N in xs) { var (a, b) = __fortup_N; … }`, N a per-file counter reset in `begin_file`) and `cmd_lift`'s format pass re-collapses it to `for ((a, b) in xs)` — sequential reuse of one synthetic name was MEASURED fine on all legs, a nested pair needs distinct names. Binders are loop-scoped: only the NEW ones are added to `declared` for the body and removed after, so a post-loop `$w = …` declares. 7 lifter tests + the `foreach_destructure` round-trip, red first for `found LBracket`; 2 parser tests updated to the enum. Six sabotages red — counter frozen (nested test + round-trip), binders leaked (post-loop test + round-trip), no keyword rename, skip-slot scan off, single binder admitted, and the hoist binding dropped: that one's FIRST form stayed GREEN (no test wrote a binder in the body), so a body-write case asserting no `CANNOT LIFT` note was added and S4c reds. Example `examples/lift/foreach-destructure` (3 legs = the PHP, draft shows the sugared loop). Scout 83 → 88/155 (`Amenities`, `Heating`, `VehicleClassifier`, `PatternMissLog`, `JobText`); `Hail.php`/`DetailHydrator.php` → named skipped-slot refusal; `JobClassifier`/`JobEmailSource` → later walls. With the by-ref hand-port, `TenureClassifier.php` stops at a PHP STATIC LOCAL (`static $keys = null;`, copy line 1494) — phorj has none: a design question. FOUND, not fixed: a positional literal list (`[['a', 1]]`, `$out[] = [$a, $p]`) is not seeded into tuples (DEC-515 seeds keyed only) — KNOWN_ISSUES §LIFT-FOREACH-DESTRUCTURE. 6C (advisor): a positional pattern over a variable known to hold NAMED tuples was a silent divergence (PHP reads missing keys as null; phorj's erasure reads the fields) — now refused by name (test written with the fix; dropping the check reds it); over a call it is disclosed, not caught; the outer-shadow case MEASURED as `E-SHADOW-LOCAL` (follow-up TBD) | M | done | 1c703dd6 | src/lift/* tests/lift_roundtrip.rs examples/lift/foreach-destructure.* |
| 5w | DEC-539 — a PHP static local (`static $x = …;`) gets a NAMED refusal instead of `` `static` is not supported in Tier-1``, naming the two rewrites (a private static field, or dropping the memo when the value is pure). Scout's one site, `TenureClassifier.php:1497`, is hand-ported in L3. BUILT 2026-09-26: `parse_stmt` matches `static` followed by a VARIABLE token only, so `static::m()`/`static fn` (next token `::`/identifier) keep their paths and a static PROPERTY (class-member parser) never reaches it. 2 lifter tests in `lifter_tests_static_local.rs`: three refused shapes incl. a method-body memo (scout's shape), and a static property + a `static fn` closure still LIFT. Red first: guard reverted → fails on the generic message (exit 101); restored via `git apply`, sha256 identical, green. Refusal documented in `examples/lift/README.md` § What lift refuses (a refusal ships no runnable example, Invariant 9). 6C (advisor): the `Var`-peek narrowing was stated but unpinned — dropping it left both tests green; follow-up `29bfc315` adds a `static::reset();` statement asserted NOT to take the DEC-539 refusal, and the peek-drop sabotage reds it. Session resumed after a machine hang: the WIP was recovered intact from the working tree | S | done | 982d0c67 | src/lift/parser/* src/lift/lifter_tests_static_local.rs |
...
- **Q-0908-2 — RULED 2026-09-26 06:27 → DEC-538, row 5u.** ~~PHP spread `...`.~~ phorj has NO spread of any kind [Verified 09-08: zero hits for
...
### Known issues
- **Two printers, two `escape_str`s (found by row 4b, 2026-09-26).** `src/lift/printer/exprs.rs` escapes any control
  character in a string literal as `\u{HEX}` (row 5n); `src/format/printer/atoms.rs` does not, so `phg format` prints a raw
  control byte where the lifter would not [Verified 2026-09-26: `"a\u{1B}b"` through `phg format`, `xxd` shows `61 1b 62`]. Row 4b shared `escape_interp` only; unifying `escape_str` (and ideally `str_lit`)
  is the same drift class and is not yet a row.
...
**Check stage** (`phg check out/src/Scout/Rent/Core/TenureClassifier.phg`; it checks its package AND the imported
`Scout\Core`, so it is the whole picture — a separate `Text.phg` run is a strict subset [Verified: 0 of its lines
absent from the entry run] and adding the two double-counts, which is how the first draft said 270). **224 distinct
errors, cascades included.** By class: 88 unknown function, 33 unknown identifier, 16 empty collection, 12 + 3 exception
`Error` shape, 9 not indexable, 5 map value types, 5 param reassignment, ~53 across ~25 smaller classes. Unknown
functions (reported calls): `sprintf` 17, `preg_last_error_msg` 10, `preg_replace` 9, `preg_quote` 9, `trim` 5,
`preg_match` 5, `in_array` 5, `str_replace` 3, `preg_match_all` 3, `min` 3, `substr` 2, `is_string` 2, `is_scalar` 2,
`implode` 2, `array_filter` 2, and one each of `uksort`, `strtr`, `preg_split`, `mb_convert_encoding`,
`mb_check_encoding`, `isset`, `html_entity_decode`, `get_debug_type`, `explode`. Unknown identifiers: `matches` 7, `m` 7,
`signals` 6, `PREG_OFFSET_CAPTURE` 6, `entity` 2, and one each of `PREG_SPLIT_NO_EMPTY`, `PREG_SET_ORDER`,
`PHP_INT_MAX`, `ENT_QUOTES`, `ENT_HTML5`. Routed to rows 4g–4k and Q-0926-4/5.

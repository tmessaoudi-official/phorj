...
| 5g0 | Found at 5g's 3C (2026-09-24, autonomous session), a P0 Invariant-7 break and a precondition of 5g: an identity cast `(x as float)` as the left arithmetic operand passed `phg check` (lint only) and was a VM compile error, while the tree-walker ran it (DEC-528). DEC-523's lift emits that shape. BUILT: a `Cast` arm in `ctype` (`src/compiler/cty.rs`). Differential `identity_cast_is_an_arithmetic_operand`, red first on the VM leg for the stated reason, green on all three legs after | S | done | - | src/compiler/cty.rs tests/differential.rs |

...
| X6 | **XML scope**: DEC-382/Q1 ruled a `quick-xml`-class DOM + C14N question; both e-invoicing codebases need XSD validation AND XMLDSig/XAdES signing, which Q1 does not cover | scope widening, not contradiction | RULED (DEC-480, 20:05: one `Core.Xml` domain incl. XSD + XMLDSig) |
...
| X8 | **Named arguments**: UNIFIED-SPEC:1209 "Phorj has no named arguments" — false for user functions/constructors (probed), true for natives only | doc wrong | FIXED (K5: UNIFIED-SPEC:1209 — natives positional; named args are DEC-297 language surface) |
| X9 | **Certification**: DEC-268 wants two consecutive clean panel rounds; the 2026-08-19 economize ruling wants one panel per milestone; round 3 returned 35 findings | decides whether a round 4 is owed after step 1 | RULED (DEC-481 fix-then-verify); round 4 ran on `bfafdd23` |
...
- ✅ **EMPTY-INTERPOLATION-POSITION (P3) — FIXED with INTERP-LINE-RESET** (same re-basing;
  `a_diagnostic_inside_an_interpolation_names_the_real_line_and_column` pins both).
...
- ✅ **INTERP-LINE-RESET — FIXED 2026-09-02** (the §5e fix shape was right; the older W0-5/W5-13 record
  said "needs VM debug symbols" and was wrong; both records now say FIXED). Original text:
...
| 4 | CD-31 / K8 residue — the PIPE rewriter's `walk.rs` made total over `Item` and `Expr`; removing the catch-alls made the compiler name three hidden variants; added to the DEC-356 anti-regrowth ratchet | M | done | PENDING | src/checker/rewrite_pipe/walk.rs src/ast/leaves.rs |
...
- Nothing hard-blocked. The charset-before-consumers hazard is CLOSED by DEC-491 AND DISCHARGED — step 6
  is built, so `Charset` exists for steps 12-14 to import. Charset is step 6,
...
- **Lifting charset conversion is OWED and deliberately not built** (Invariant 17 names lift a
  ...
  slice (step 13), which needs the same alias table for RFC 2047.

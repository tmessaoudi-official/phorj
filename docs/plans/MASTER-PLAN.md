...
|---|---|---|
| DEC-504 | Structural **named-field tuples** `(tenure: Tenure, source: string, bp: int)` — the phorj form of PHP's keyed `array{…}` shape, 73 scout sites and the largest lift wall | RULED 2026-09-07 (build shape 2026-09-08) — **BUILT as L4, 2026-09-08.** Field order is part of the type; the PHP leg is positional; `t.bp` reads a field and `var (a, b) = t` still binds by position. The Invariant-7 trap was real and had **THREE** consumers — the VM compiler, the TRANSPILER (which picked the wrong PHP *operator*, printing `31` for `4`) and INFERRED locals. Lift closes the wall: a keyed shape lifts to the named tuple its keys describe, and `$row['bp']` lifts to `row.bp`. ⚠ **PERF OWED but NOT the feature's — RE-MEASURED at K=7, interleaved and core-pinned** (the original K=1 unpinned numbers are withdrawn): `namedtuplefield` **0.25×** and its erased-form control `nestedlist` — now shipping as a permanent pair — **0.03×**, with the two VM legs 0.4% apart, so the feature costs ZERO over its erased form. The loss is phorj's TWO-LEVEL list element read (370 ns/iter vs 4.3 one level, 1.4 pure arith); not a deep copy, and the JIT does not close it. ⚠ **Lift ceiling re-counted: 140 rewritable reads / 16 files, not 1,324 — and the SHIPPED param-seeded rewrite covers 0 of them** (scout's keyed shapes are iterated collections; the binder is what gets indexed). The wall-closing is the delivered value; reaching the 140 is now RULED as **DEC-515 / lane L4b**, which blocks L3. ✅ **The wall-closing is now MEASURED on scout, not inferred** [2026-09-08 whole-tree lift]: **57/123 → 64/125**, **zero `array{…}`-shape refusals** (was 16 files), and `Rent/Core/Classification.php` — the four-leg contract file that blocked L3 — lifts, its `toArray()` typed as a named tuple. It does not yet CHECK: the body still returns the keyed array literal, the same seed gap on the WRITE side. **DEC-507 escalation RULED 2026-09-09 as DEC-514: fix the nested read BEFORE L3, root-cause first** |
| DEC-514 | The DEC-507 escalation L4 owed: `namedtuplefield` **0.25×** and its erased-form control `nestedlist` **0.03×** vs docker php:8.5+JIT (K=7, interleaved, core-pinned), VM legs 0.4% apart — so DEC-504 costs ZERO and the loss is phorj's TWO-LEVEL list element read, 370 ns/iter against `listindex` 4.3, not a deep copy. **ROOT-CAUSED 2026-09-09**: the origin is a HOLE in the unboxed JIT `Kind` lattice — it has a `MapList` and a `SetList` but no list-of-lists — so `admit_make_list` refuses the row literal at `Op::MakeList`, before any index runs, and the function falls to the VM (there is no boxed fallback). `listindex` compiles and emits no decline at all. The prior clause "JIT-resistant" / "the JIT engages without closing it" is RETRACTED — it does not engage. | RULED 2026-09-09 — **fix it BEFORE L3, ROOT-CAUSE FIRST** (Rule 14). Lane **L4c**; L3 is blocked behind it. Departs from DEC-513's "carry the OWED and proceed" deliberately: that residue was attributed, this cliff is not, and L3's oracle benches the exact `list<array{…}>` shape it appears on. Rejected: carrying the OWED; ruling it outside DEC-507's artifact scope |
| DEC-515 | The lift CEILING behind the fallen wall: the tuple-field rewrite is seeded from PARAMS only, so it covers **0 of 140** rewritable reads (16 files), and `Classification.php` lifts to a named-tuple return type whose body still returns the keyed array literal, so it does not check | RULED 2026-09-09 — **extend the seed NOW, before L3**. Lane **L4b**: `@var` locals + `foreach` binders over a keyed collection (reads), keyed array literal in a named-tuple return position (writes). No new language ruling; DEC-166 untouched, since a declared element type is not an invented one. **HALF BUILT 2026-09-13** — binder reads + return-position keyed literals landed (~71 reachable reads, not 140); the `@var`-local half is scout row 5d (**BUILT 2026-09-25**; only `Dedup.php` reachable in scout, whose per-key write needs a tuple copy-and-update form — Q-0925-2), and `Classification.phg` is now blocked by causes outside DEC-515, which gate L3 — rows 5e–5j of the scout plan, re-scoped 2026-09-13 when the "same-package import" cause was refused by measurement (same-package types resolve through the entry flat-merge): `usort`/`array_map` (5f, BUILT 2026-09-13), PHP `/` (DEC-523), UFCS outside `package Main`, lowered enum methods vs `E-FILE-MIXED-PUBLIC`, readonly ctor assignment, checking a library file alone |
| DEC-505 | **`<=>` operator + lexicographic tuple/list ordering** — both halves missing today; transpiles to PHP's own `<=>`; `List.sort` must be proven stable | RULED 2026-09-07 — **BUILT** (`eba09d4e` ``feat(lang): `<=>` and tuple ordering``, `ab959542` `docs(lang): L2 closes`) **as amended by DEC-512** — the list half did NOT ship |
...
| DEC-513 | **The DEC-507 loss on `<=>` is FIXED, not carried — FUSE the comparison in the compiler.** `spaceshipsort` measured 0.36-0.37x vs docker php:8.5+JIT; attribution puts ~65% of the cost in materializing two tuples per comparison, not in `Op::Cmp` (tuple 169 ms / scalar 60 ms / subtraction 56 ms). Lower `(a,b) <=> (c,d)` element-wise, reusing `value::compare_ord`/`three_way` so Invariant 4 holds; transpile leg unchanged | RULED 2026-09-08 — **BUILT as L2b; structural goal met, perf bar STILL MISSED (OWED).** `Op::CmpSeq(n, SeqOrd)` compares elements in place, no tuple materialized (A/B disassembly proof). Measured A/B on one quiet box: **0.38x -> 0.46x**, VM time -19.5%, still ~2.2x php. Materialization was ~a fifth of the tuple-vs-scalar gap, not ~65%; the residue is per-element `compare_ord` dispatch INSIDE the fused Op, not the `sortWith` callback (which the scalar comparator pays too) — so it is compiler-reachable, not L8. Register carries the correction. **Developer ruled 2026-09-08: carry the OWED, proceed to L4; L2c NOT authorized** |
...
| DEC-516 | The G-8 ratchet baseline is unreproducible and its fallback accepts an invalid build: `_baseline_php` names a phpbrew `php-8.5.8` that no longer exists, `--emit` rewrites all 53 rows in one run, and the surviving `ZTS DEBUG GCOV` oracle passes the gate's JIT-only probe while DEC-507 calls that build invalid for a perf claim | RULED 2026-09-09 — **re-emit on dockerised `php:8.5-cli`+JIT, core-pinned, quiet box; add a `PHP_DEBUG` refusal to the resolution chain.** SUPERSEDES DEC-423.1's local-php baseline. **BUILT 2026-09-09** — and the finding sharpened: the debug-fallback hole was LATENT (its banner appears 0 times in the push logs), while the LIVE defect was docker-measured runs compared against a phpbrew-recorded baseline. The gate now refuses that comparison too (disclosed scope extension); DEBUG refuses, ZTS warns |
| DEC-517 | Invariant 17's 100% diagnostics rule REOPENED — the surface ratchet reports 317/321 (98%) and the four unasserted codes are all four of L2's ordering codes, while SLICE-STATE still records the rule as CLOSED at 311/311 and L4's row reads `done` with its LSP references / document symbols / signature help unprobed | RULED 2026-09-09 — **fixture the four, re-emit the floor, correct both ledger claims.** Build QUEUED |
| DEC-518 | The rule-drift pass: this file carries DEC-387's retired *plain-text questions only* protocol at `:1113` as LIVE text (two `never push` lines were also flagged and the flag was WITHDRAWN — both were already annotated as superseded in place); the certification tier has been decided by a sentinel for three weeks unratified; `CLAUDE.md` and `docs/INVARIANTS.md` number their invariants differently AND the numbers collide; Invariant 13's 500-line hard cap is a frozen ratchet with 56 files grandfathered over it | RULED 2026-09-09 — **all four, one docs-only commit, in that order.** The numbering fix is a PREFIX (`T-1`…`T-14` on `docs/INVARIANTS.md`), not a renumbering, so nothing existing is renamed. · ✅ **BUILT 2026-09-09 — `36fa37d4`** (plan row 12); the `never push` half of item (1) was found ALREADY annotated as superseded and is WITHDRAWN — corrections `a789ec32` + the review's own defects pass |
| DEC-519 | DEC-514 scoped L4c to the `nestedlist` / `namedtuplefield` pair, but four carried OWED micros are element-access-heavy loops over the same shape | RULED 2026-09-09 — **widen L4c's root-cause phase** to measure `fslines` / `queryparse` / `fsforeachline` / `strappend` as corroborating evidence before the hypothesis. AMENDS DEC-514's scope; its root-cause-first requirement is untouched. Lane order unchanged. ⚠ **PREMISE RETRACTED 2026-09-09**: none of the four contains a two-level list read. The widening still paid off as ruled — a decline census found FOUR distinct causes, and `strappend` COMPILES (so its loss is NOT JIT coverage; what it IS was not examined here) |
| DEC-520 | L4c's root cause is landed (`c8bc6c43`) and the fix was deliberately not written — its shape is a user-visible engine decision, and sabotage proved it is a vertical, not a one-line admission | RULED 2026-09-09 — **INT-ONLY inner lists**: a dedicated list-of-lists `Kind` fixed to `int` elements, inner read reusing `IntList`'s flat encoding. Narrowest change covering `nestedlist` / `namedtuplefield` / scout's lifted `list<array{int,int}>` (all `List<List<int>>`); the general case can grow from it. 10 code sites + the ABI decode gate in `compile/mod.rs`, size M-L. REJECTED: a general element kind / widening `DynList` now; taking L4b first. **BUILT 2026-09-09** (`8e814277` + `017df010`): `Kind::IntListList(Own)` across 12 code sites in 4 files; the ABI decode gate needed no edit (it is an allow-list) and the two zero-headroom emit files forced an M-Decomp split as a separate first commit. The 4 decline pins flipped to compile + `hits > 0` + oracle identity; a 5th pins the kept boundary. **L4c CLOSES; L3 is still blocked behind L4b.** Perf before/after OWED (box load 20.07) |
| DEC-521 | The developer asked (2026-09-13) to upgrade every dependency — toolchain, crates, CI tools, editor client, playground vendored JS | RULED + BUILT 2026-09-13 except the edition — Rust 1.98.1, lockfile, `argon2` 0.6 (0.5.3 hash pinned first), `fancy-regex` 0.19.2 checked against PHP (two new divergences, `\O` and `(?~…)`, now refused on every leg; some catastrophic PCRE-class patterns now resolve natively where the PHP leg still faults — the disclosed PHP-leg limitation, widened), wasm-pack 0.15.0, VS Code client 10.1.1, CodeMirror 6.0.2, `php-wasm` 0.1.0; `cranelift` 0.135.2 in its own commit (no perf claim yet); **edition 2024 with formatting** QUEUED as its own slice. Plan: `docs/plans/2026-09-13-dependency-upgrade.plan.md` |
| DEC-522 | CI redded master on the DEC-521 push: PHP's reading of `{,n}` and of spaces inside `{n,m}` depends on the host's PCRE2 (a quantifier from 10.43, literal text on CI's 10.42) | RULED + BUILT 2026-09-13 — refused on both constructors on every leg (`pcre_divergent` + its PHP twin); `{,}` stays literal; the `x{,2}y` pin removed. Plan: `docs/plans/2026-09-13-dependency-upgrade.plan.md` |
| DEC-523 | The lifter maps PHP `/` to phorj `/`, which is integer division on two ints, so scout's `confidence(): float { return $this->confidenceBp / 100; }` lifts to a body that fails `phg check` (`expected float, found int`) | RULED 2026-09-13 — **int/int PHP `/` lifts to float division** `(a as float) / (b as float)`, int literals as float literals; an exact division into an `int` slot fails `phg check` loudly instead of printing a wrong number. REJECTED: type-directed (Invariant 14 case 3), refuse by name. Build QUEUED as scout row 5g |
| DEC-524 | A PHP `readonly` property computed in the constructor body (scout `TenureSignal::$length`) has no phorj form: assigning an immutable field outside its initializer is `E-ASSIGN-IMMUTABLE` | RULED 2026-09-13 — **the ctor-once rule**: an immutable field with no initializer may be assigned in its own class's constructor body; anywhere else stays `E-ASSIGN-IMMUTABLE` (PHP `readonly`, Java `final`, C# `readonly`, Kotlin `val`, Swift `let`, TS `readonly` all allow it). Definite-assignment shape surfaced at the build's 3C. REJECTED: lifting to `public private(set) mutable` (drops the in-class guarantee), refuse by name. Build QUEUED as scout row 5j (L) |
| DEC-525 | `phg check <file>` on a library-package file loads no siblings, so an entry-less package cannot be checked file by file, and the lift report's hint says it can | RULED 2026-09-13 — **a checked file loads its whole package** plus its imports, as an entry would; the report hint becomes true and editor diagnostics share the view through `load_with_buffer`. REJECTED: a `phg check <dir>` mode, a hint-only fix. Build QUEUED as scout row 5e |
| DEC-526 | DEC-509 lowers a PHP enum method to a public free function beside the public enum, which is `E-FILE-MIXED-PUBLIC` in any package but `Main` (scout's `Tenure.php`); `internal` cannot help because scout's callers are sibling packages | RULED 2026-09-14 — **the lifter writes lowered methods to a sibling `<Enum>Functions.phg`** in the same package; callers use UFCS (row 5h) or a function import. REJECTED: a companion-function exemption from `E-FILE-MIXED-PUBLIC`, enum methods as a language feature. Build QUEUED as scout row 5i |
| DEC-527 | UFCS on a user free function fails outside `package Main` (bare-name lookup against mangled keys), and `private` file scope lives only in the loader | RULED 2026-09-14 — **UFCS accepts exactly what a direct call at the same site accepts**: same-package functions resolve outside `Main`; a `private` one is visible in its own file and `E-VIS-PRIVATE` elsewhere, via a loader-supplied file span window. Cross-package UFCS through a function import DEFERRED to L7 with a loud-pinning test. REJECTED: refusing `private` through UFCS, refuse-now-exact-later. BUILT 2026-09-14 as scout row 5h — red first also exposed a SILENT half: a library's `s.shout()` ran `Main`'s `shout` when one existed; fixed by the same key |
...
attribution (comparator body swapped on a pinned core) puts ~65% of the cost in materializing two
tuples per comparison, with `Op::Cmp` itself at parity with an integer subtraction. Per DEC-365 it is
...
the lifter maps spread over known lists to `List.concat`/`List.flatten`, no new syntax (scout row 5u, ahead of L3).
...
**DEC-537** (an assignment inside a narrowed block checks against the DECLARED type) — both RULED 2026-09-25 19:02; DEC-537 BUILT as scout row 5t (direct statements only, scope ruled 20:01), DEC-536 BUILT as row 5s (local places; class-field/call bases stay refused).
...
| DEC-486 | Panel C9/K1/K7 — the LSP and `phg check` hard-code the non-test checker, so every `selftest/*.phg` squiggles… | BUILT 2026-09-02 (`0356d04d`) |
...
`Core.Sandbox` RULED for v1, QUEUED not built (pure-expr, tree-walker-only, `E-TRANSPILE-SANDBOX` — zero src hits yet); ArrayAccess
...
mapping — multi-round research with the dev). **DEC-336 extensionless `#!`-shebang sources +
perpetual editor/LSP currency — ✅ BUILT 2026-07-24/25** (100%-clear tooling slice: a `#!…phg` first line makes an
...
1.99× re-verified in the same run). **CAMPAIGN CLOSE + DEV-BOX RECONCILIATION (2026-07-23): the canonical ledger is 44 WIN /
...
7. **Usability + shippability:** the **lifter** spearhead (Tier-2 mappings + real-app corpus —
   ...
   **Project-aware lifting SHIPPED (DEC-439 parts 1+2, 2026-08-05):** `phg lift <dir> -o <out>`
   ...
   vendor is detected from composer's full autoload surface + `installed.json` and REPORTED by default, with
...
enrichment). Full oracle gate PHORJ_REQUIRE_PHP=1 green (1990). **DEC-208 S2 ✅ SHIPPED** (`src/checker/desugar_db.rs`): the typed-
generic `queryInto<T>()`/`queryOneInto<T>()` hydration (type-directed — checker resolves T's field layout,
a `DbSys` native hydrates by strict name → `DbError`; same result-value protocol). **DEC-220 S2/S3 ✅ SHIPPED
(2026-07-14 — DEC-220 fully shipped S1+S2+S3)**: `Response` builders (+ serve Output→stderr redirect removed) · capture landed as `Output.capture` (the ruled `Response.capture` wrapper was dropped — no leak-free path; see DEC-220-S3 in the register).
...
> Detected in the post-audit gap session. Current position (§4.22, 2026-09-25): **parity ≈71% · vision ≈73% ·
> raw floor ≈59%** (§4.13 recompute, 2026-07-30 — backed enums + phantom-gap credit; §4.10 was ≈66/67/51). Vision = 0.70×parity + 0.30×programme (§11.1, ratified).
...
**G-1 · Byte-identity spine.** `phg run` ≡ `phg run --tree-walker` ≡ transpiled PHP under a real `php` (floor
...
(4737 lines as of 2026-07-28 — split tracked as W1-1) + `tests/conformance.rs`.
...
container. That container is gone; on this machine the tool works, `askUserQuestionTimeout` is
`"never"` globally, and the global Stop hook now REQUIRES it, so following DEC-387 would break the
...
800/1000 with 12 tracked over-cap files; `scripts/size-gate.sh` shipped 2026-07-20 and is pre-push-wired; only the CI-workflow wiring remains — W1-6.)
...
- **D (developer-ruled 2026-09-03, DEC-493) — the 100 % bar is LITERAL: NO PARKS AT THE END.** Rule 3
  ...
  parked rows. **This BOUNDS DEC-269 to in-flight use**: DEC-269's own text states no terminal
  clause — a LOSS-FLAGGED micro with anatomy and queued levers remains a valid per-feature
  definition-of-done *in flight*, and stays one; what ruling D pins is that a flag is not an acceptable
  FINISH state. Keep writing LOSS-FLAGs; just do not ship one.
...
### The 2026-07-11 snapshot (SUPERSEDED — current figure is §4.11 / §0)

> ⚠ **SUPERSEDED.** The CURRENT parity/vision figure is **PHP-parity ≈68% · Vision ≈69% · floor ≈53%**
> (`M-gap-matrix.md §4.11`, 2026-07-19; mirrored in §0). The 2026-07-11 numbers below are retained only
> for the road-to-100% ROW BREAKDOWN, which the later recomputes did not re-tabulate.
...
- **Ω-0 · Footgun audit — ✅ DONE (session 5, 2026-07-12):** all 49 GD rows walked one-by-one —
  ...
  `Core.ClosableModule`; `defer` stays rejected on its merits; lift was deferred, and **LIFT-TRY is now
  BUILT 2026-07-31** — `try`/`catch`/`finally` is in the lift subset, so what remains is only the
  narrower "raise a try/finally back to `using`" shape recognition, deliberately not guessed; see
  KNOWN_ISSUES §LIFT-USING). `onShutdown` build still queued. DEC-383's later fork (c)
...
- ☑ B3-1 "zero external deps" FALSE everywhere — **CLOSED by the 2026-07-28 consistency audit**
...
`deny(unsafe_code)` both crate roots (sole audited island: `src/jit/`, CI-gated); kernels single-sourced; deps exactly per policy; all prior
...
- **M5 — NEW: editor support so the project is actually writable/testable in an IDE:**
  ...
  - **PhpStorm** (`editors/phpstorm/`): **FINDING (2026-07-04) — the "README-ONLY, no working path"
    ...
    **LSP4IJ** marketplace plugin running `phg lsp` for diagnostics/hover/completion/rename/format.
    ...
    Refreshed the docs (dead `phg fmt`→`phg format` verb) in this pass. **Scope decision — since
    RULED by DEC-181 (2026-07-04): LSP-first both editors + thin native shells now, the full
    native IntelliJ/PSI plugin as a follow-on phase.** The question as recorded: should the run
...
- **W2-3** ~~foreach REPLACES for-in~~ **AMENDED by DEC-343 (2026-07-26): KEEP BOTH** (DEC-248 superseded on this point; Conflict C-2 closed); add cross-form migration hints + binding forms. No retirement codemod.
...
intact in phorj's own code (then-`forbid`; since relaxed to `deny` roots + the audited JIT island). Pure zero-dep P0s ship FIRST (`Core.Sql`, `Core.Url`). Recorded in
...
- **W4-10 · XML/DOM/XPath** (L) — **RULED as DEC-382 (2026-07-26): admit a vetted
  `quick-xml`-class crate as the 15th dependency — research + build queued** (the 2026-07-03
  PENDING adjudication artifact is answered; the "PHP-DOM-backed" recipe is superseded by the
  vetted-crate ruling; `Cargo.toml` + UNIFIED-SPEC policy row update in the same change).
...
- **W5-13 · VM debug symbols** (L) — scope IP ranges → named locals → per-line pause → DAP over
  the VM; fixes the interpolation fault-line divergence (flip the W0-5 harness flag on).
...
> ⚠ **SUPERSEDED (2026-07-25 note).** This §11 ledger tail (60% / ≈63%) was the 2026-07-12 re-pass and
> was never re-tabulated afterward. The recompute chain CONTINUED in `M-gap-matrix.md §4.6→§4.11`
> (61 → 62 → 64 → 66 → **68% parity / ≈69% vision**, 2026-07-19) — the LIVE figure, mirrored in §0.
> Treat `M-gap-matrix §4.11` (and §0) as the parity-% SSOT; a fresh §11 re-projection from the 68%
> baseline is owed (recompute-per-milestone rule) and remains a developer-owned reconciliation.
...
> **DEC-197 (2026-07-05, developer-proposed — scope since settled by DEC-274, 2026-07-16: the sugar-gate discipline, ruled "everywhere").
...
- **DEC-200 — top-level type named after a PHP-reserved-as-class word (surfaced 2026-07-06 —
  since RULED as DEC-202 and SHIPPED 2026-07-13: `E-RESERVED-NAME` rejects ~100 always-loaded PHP
  builtin class names via `is_php_builtin_class_name`; DEC-386 closed this row as already-ruled).** The enum-*variant*
...
| Concurrency PHP leg | hard error (shipped code: `E-CONCURRENCY-NO-PHP`); the `--sequential-concurrency` opt-in was never shipped and DEC-369 deleted it from the rule |

...
- [2026-07-05] **EXAMPLES/CONFORMANCE AUDIT + cleanup decisions (DEC-196; audit = `docs/research/2026-07-05-examples-conformance-audit.md`).** Developer-ruled this session:
  - **Q1 [FIX, this session]:** rename `examples/fmt/`→`format/` AND `examples/bench/`(+`manual/`)→`benchmark/`; fix `bench.rs:339` output `"phg bench —"`→`benchmark` (+2 tests); fix `examples/benchmark` READMEs/comments (`phg bench`/`disasm`→full verbs); add `import Core.String;` to `web/core-http.phg`; reconcile `STABILITY.md` module names→real (`Output/String/Conversion/Validation/Reflection/Environment`); `git rm docs/plans/wave0-remainder.plan.md` (superseded straggler); sweep ~15 `src/**` `phg fmt`→`format` doc-comments.

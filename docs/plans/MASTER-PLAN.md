...
| **Actively in progress** | **Wave A slice 1 = PRIMITIVE `match` type-patterns** (HEAD `2300052`; SSOT merge committed). Failing test captured: `match (x) { int i => …, string s => … }` on `string\|int x` → `E-MATCH-TYPE: type pattern 'int' must name a class or interface`. Parser ALREADY makes `Pattern::Type{type_name:"int"}` (primitives are idents). NO-NEW-OP design — 4 sites: (1) checker `src/checker/matches.rs:330` allow int/string/float/bool/null type-patterns + type the binding + primitive-union exhaustiveness; (2) VM `Op::IsInstance` (`src/vm/exec.rs`) recognize primitive type names at runtime (compiler `src/compiler/matches.rs:114` already emits IsInstance for type-patterns — reuse); (3) interpreter match evaluator same dispatch (oracle); (4) transpile `src/transpile/matches.rs:310` primitives → `is_int($x)`/`is_string()`/`is_float()`/`is_bool()`/`is_null()`. Then slices: union-element collection methods+foreach → `is` flow-narrowing → W5-3 sealed hierarchies. Each: full differential + Invariant-9 example + both-editor LSP. **BYTE-IDENTITY DESIGN CONSTRAINT (slice 1):** primitive type-tests discriminate only the PHP-`is_*`-distinguishable set — `int`/`float`/`string`/`bool`/`null` (→ `is_int`/`is_float`/`is_string`/`is_bool`/`is_null`). `decimal`/`bytes`/`html`/`attr` all ERASE to PHP `string`, so a union mixing `string` with any of them is indistinguishable in the PHP leg → reject that type-pattern with a clear error (byte-identity-forced, LADDER-style); tracked as a PENDING note, not silently downgraded. Build mode = AUTONOMOUS MARATHON (commit each green slice, never push; NEW §15 sub-fork → PENDING question). **SIDE — Pages CI (re-diagnosed from live deploy log):** build SUCCEEDS (artifact created); failure is `deploy-pages@v5` "Getting Pages deployment status" → "Deployment failed, try again later." = GitHub Pages BACKEND-side (NOT wasm-pack/A-CI-4, NOT source). Likely transient OR `cancel-in-progress` interrupting a deploy. Action: RE-RUN the failed job first; no verified defect in the workflow YAML. |

...
| **Next** | **Ω-8 is CLOSED** (sweep done, representation slice recorded). NOW: perf register + G-8 recompute (824-row re-pass due at this wave close) → **Ω-0 footgun audit → Ω-1…Ω-9** under the PER-FEATURE PERF GATE (every new feature ships its own ≥1.0× micro in the same change). |

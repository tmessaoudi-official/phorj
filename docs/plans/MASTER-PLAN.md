...
| | |
|---|---|
| **Date / HEAD** | 2026-07-12. **🏆 PERF-100% FLIP PHASE COMPLETE — ALL 17 micros ≥ 1.0× vs fresh php:8.5-cli+JIT** (protocol medians): trycatch 32.6 · objalloc 9.3 · match 7.2 · floatarith 4.0 · methodcall 2.9 · webish 2.2 · strbuild 2.2 · interp 2.5 · closurecall 2.1 · stringconcat 1.9 · fibrec 1.9 · enum 1.6 · **intadd 1.48 (checked-default beats php's unchecked)** · listindex 1.47 · mapget 1.01 · floatloop 1.01 · floatmul 1.00. Session-4 verticals: trycatch native throw/catch, inline mixed interpolation, ACC-record accumulator, packed map buckets, task-9 interval elision. Prior: 2026-07-11 plan/spec consolidation. |
...
| **Next** | Ω-8 sweep tail: **fundamentals micro sweep** (collection writes, capturing closures/HOF pipelines, iteration — every new micro must reach the bar) → representation slice (V3b + Rc cycle-leak; fork → KNOWN_ISSUES per the overnight directive) → perf register + G-8 recompute → Ω-0…Ω-9. |

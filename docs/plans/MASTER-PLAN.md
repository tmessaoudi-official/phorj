...
| **Next** | **Ω-1 gate levers: 6 of 8 W-slices SHIPPED** (`b59d9b0` general List.append clone · `021317c` native bridge2 join/contains/splitOnce/drop + str Eq/Ne · `8c1deb7` handle args/returns + builder-method Inst returns + entry str/list return materialization · `a5cac4a` fixpoint un-poisoning (pre-pass no longer stamps ctor-arg params Int; Int⊔handle join REVERTED as union-unsound) + `JitCache.redos` observability · `e4039b5` Bool consts + `Conversion.toString` · `c8cf1dc` handle-LIST/map instance fields). **REMAINING (fresh context — spine-sensitive): (W7) union params as 2-word Dyn cells** — `Kind::Dyn`, tag word rides `evars` (EnumInt precedent: tags 0=int 1=float-bits 2=bool 3=str-handle), call-ABI expands a union param to 2 slots, consumers = DynList push-by-tag helper + MakeList-mixed arm (the `binds` list is `List<union>` — this is bench's current blocker: "MakeInstance field kind Unknown" via the union chain); **(W8) two-slot instances** (> 8 fields ≤ 15: fields 0..6 in slot A, A[7] = slot-B index word, 7..14 in B; MakeInstance allocs 2, release frees B then A, Get/SetField two-hop for j ≥ 7 — SelectQuery has 11 fields); **(S8) prelude JoinClause flattening** (JoinClause must copy the parent SelectQuery's FIELDS — an instance-kind field would dangle when the chain frees the receiver after `.on()`); then measure sqlbuild (protocol), ratchet at ≥ 1.0×, THEN Ω-1 continues (Core.Db rusqlite, HTTP client, sessions). |

...
| **Completion** | **PHP-parity ≈66% · Vision ≈67% · raw floor ≈51%** (§4.10, 2026-07-19 — overnight Wave-B: named args + variadics + Math tail + List set-ops + String tail + Deque + PriorityQueue; §4.9 was ≈64/66/47, Web/Runtime spine). Weights 35 SYN(≈83%) / 40 FN(≈45%, still the drag) / 25 RT(≈75%) on 824 rows. ⚠ PHANTOM-GAP finding (§4.10): several §1.2 "gaps" already built (Core.Path/FS-broad/crypto) → true parity higher, pending the owed per-row §1.2 re-pass. The ONE big lever remains the FN stdlib leg; next FN blockers = XML/streams/intl/SPL-heaps/mb-tail. |
...
> Detected in the post-audit gap session. Current position: **parity ≈66% · vision ≈67% ·
> raw floor ≈51%** (§4.10 recompute, 2026-07-19; §4.9 was ≈64/66/47). Vision = 0.70×parity + 0.30×programme (§11.1, ratified).

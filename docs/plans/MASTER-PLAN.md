...
| **Next** | **Ω-0 ✅ · Ω-1 slice 1 (Core.Sql instance rework + method `?`-throws) SHIPPED `a3031e0` — its per-feature gate is OPEN: sqlbuild 0.30× (honest, disclosed, NOT ratcheted).** ACTIVE WORK = the PRELUDE-SHAPE JIT WIDENING (the gate's lever, unblocks every Ω library feature): (L2-fix) per-call `UbCtx` setup makes JIT'd handle-methods SLOWER than `--no-jit` on many-call shapes (2.90s vs 2.06s on sqlbuild) → reuse/pool the ctx across calls; (W1) Inst fields widen to LIST/handle kinds; (W2) boxed `Value::List` handles in the subset (new/push-by-tag/clone-append/len helpers — the union-element `binds` list); (W3) union-typed params as 2-word (tag, payload) cells (EnumInt precedent; unions only FLOW here — no arithmetic); (W4) `String.join` + remaining native arms. Measure after each; ratchet sqlbuild when ≥1.0×. THEN Ω-1 continues (bindNamed ✅ done in slice 1; next: `Core.Db` rusqlite Tier-B, HTTP client, sessions). |

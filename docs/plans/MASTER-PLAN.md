...
| **Completion** | **PHP-parity ≈62% · Vision ≈64% · raw floor ≈42%** (§4.8, 2026-07-16 — RECOMPUTE OWED: this session shipped Unicode/stdin/Cookie/string-distance + the whole extension architecture, none folded in; a fresh §4 pass is queue item 0 below). Weights 35 SYN(≈80%) / 40 FN(≈37%, the drag) / 25 RT(≈75%) on 824 rows. The ONE big lever = the FN stdlib leg. |

...
| | |
|---|---|
| **Date / HEAD** | 2026-07-12. Ω-8 verticals: trycatch NATIVE throw/catch (**33.4× WIN**) + fully-inline mixed interpolation (**webish 2.24× · interp 2.65× WINs**) — map now **12 WINs / 17 micros** (remaining losses: strbuild 0.42 · intadd 0.71 checked · mapget 0.85; tail listindex 0.98). Prior: 2026-07-11 plan/spec consolidation (perf-wave / di-attributes / web-spine / cli-name-sync / finishing-wave folded into this file + UNIFIED-SPEC + KNOWN_ISSUES; ROADMAP→thin pointer). |
...
| **Next** | Ω-8 flip-seq continues: **strbuild inline in-place append** → mapget probe micro-tune + listindex emit tail → checked-intadd elision (task 9) → fundamentals micro sweep → representation slice (V3b + Rc cycle-leak §15 fork) → perf register + G-8 recompute → Ω-0…Ω-9. |

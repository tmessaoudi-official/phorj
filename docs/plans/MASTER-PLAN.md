...
- **W3-5 · `String.format` (sprintf family)** — design RULED (developer, 2026-07-03): lives on
  ...
  **BUILD = a fresh-context WAVE (spine-sensitive):** the hard part is a Rust `%`-sprintf renderer that is
  **byte-identical to PHP `sprintf`** (interp + VM must match the transpiled PHP `sprintf` leg — PHP sprintf
  has many quirks: `%g/%e/%b/%o/%x`, width/precision/flag combos, rounding). Slice by conversion set
  (`%s`/`%d`/`%f`+width/precision/`-`/`0` flags first; `%x/%o/%b/%e/%g`/`%N$` positional later), each slice
  byte-verified run≡runvm≡php-8.5.8. A dynamic (runtime) spec is supported (no collision); an unsupported
  conversion → clean fault. Do NOT tack onto a long session. (Superseded DEC-198's compile-time
  desugar-to-`Str` architecture — that was `{}`-specific; `%` needs the runtime renderer instead.)

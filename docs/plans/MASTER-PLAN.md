...
| **Next** | Ω-8 flip-seq continues: **mapget probe micro-tune + listindex emit tail** → checked-intadd elision (task 9) → fundamentals micro sweep → representation slice (V3b + Rc cycle-leak §15 fork) → perf register + G-8 recompute → Ω-0…Ω-9. |

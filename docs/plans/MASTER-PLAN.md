...
| **Next** | 🔴 **BLOCKED ON 27 DEVELOPER RULINGS + ONE P0 (2026-07-25 global review).** Read `docs/research/2026-07-25-completeness-register.md` §2 for the agenda (`GR-1`…`GR-27` ⇄ **DEC-339…DEC-365**, 27 rows, all PENDING in the decision register — the agenda is split across §2/§6.4/§7.3/§8.4, so read §2's FULL AGENDA INDEX); raw evidence in `docs/research/2026-07-25-global-review/`; live cursor detail at the top of `SLICE-STATE.md`. **P0 (DEC-339/GR-1) is now RULED (2026-07-26) and QUEUED FOR BUILD, not open:** shadowing a live outer local/param in ANY nested block breaks the Invariant-1 byte-identity spine (phorj has true block scoping, PHP has none, so the emitter clobbers the outer binding; uncaught because the differential globs `examples/**` and no example shadows). **Ruling: reject redeclaration of a live local or parameter binding** (same scope or enclosing; class fields never conflict; a lambda starts a new function), enforced in the **checker** — alpha-renaming was considered and rejected. Probing widened it from 6 shapes to **10** (new: `for…in` loop variable, `match` arm bindings, binding-`if`, `catch` bindings; nested `for` reusing a counter changes **control flow**). **Canonical rule + full 23-row case list: `docs/specs/2026-07-26-block-scope-shadowing.md`.** Two adjacent breaches recorded separately: **DEC-366** (`phg lift` emits non-compiling phorj for function-scoped PHP) and **DEC-367** (`implements Error` + `getMessage()` → PHP `Fatal error`). This pass discharges the already-RULED **DV-5** completeness sweep. **Then** resume §0 THE QUEUE below. Prior in-flight items still stand: perf (jsonround/dbwork → wins + baselined), the §4 recompute. (The former sqlbuild-JIT-saga text is retired — captured in git history + [[w7-union-dyn-cells]]; the bytecode-inlining/scalar-replacement blueprint remains a parked general JIT lever under DEC-266.) |
...
perpetual editor/LSP currency** (100%-clear tooling slice: a `#!…phg` first line makes an

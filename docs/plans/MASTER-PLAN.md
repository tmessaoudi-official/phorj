...
- [2026-07-05] **EXAMPLES/CONFORMANCE AUDIT + cleanup decisions (DEC-196; audit = `docs/research/2026-07-05-examples-conformance-audit.md`).** Developer-ruled this session:
  ...
  - **Q3 [DESIGN, W2-6 — ⛔ RE-OPENED 2026-07-05: SOURCE CONFLICT, PENDING developer adjudication].** Move the four intrinsics behind explicit imports. **Investigation for the build surfaced a contradiction on a user-visible surface (bare vs qualified call syntax) between two authoritative sources — an autonomous session may not self-rule it (Invariant 15).** (a) **DEC-196 Q3 (this ruling, 2026-07-05):** SPLIT into `Core.Assert` (`assert`) + `Core.Abort` (`panic`/`todo`/`unreachable`), **called BARE after import** (`import Core.Assert;` → `assert(x)`). (b) **UNIFIED-SPEC §"Nothing in the wind" / W2-6 (PRINCIPLE IN FORCE):** single `import Core;`, **called QUALIFIED** (`Core.assert(x)`, `Core.panic(x)`), error `E-UNIMPORTED` — rationale *"qualified (attributed to the core) AND imported (nothing in the wind)"*. (c) **Audit §1.11 (same 2026-07-05 material) records the developer's own instinct: "bare reads inconsistent"** — i.e. pro-QUALIFIED, contradicting DEC-196 Q3's "used bare". So 2 sources (spec principle + developer instinct) point qualified, 1 (latest explicit line) points bare. **Build facts already established (all checker-only, no §15):** `import Core.Assert;`/`Core.Abort;` are ALREADY accepted (no loader existence-check today → no loader change); the gate is a checker-only guard in `check_intrinsic_call` (assign.rs:454) reading `self.imports`; backends unchanged (native-less module import emits nothing; intrinsic lowering is post-checker); only 2 example files use intrinsics (`guide/assertions.phg`, `guide/result.phg`); `is_intrinsic_name` reservation stays. **Ready to build the moment (bare | qualified | split-qualified-by-leaf) is ruled** — surfaced via AskUserQuestion this session. Distinct from `Core.Test.assert`. Best executed once ruled (breaking).

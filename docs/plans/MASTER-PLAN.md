...
| **Completion** | **PHP-parity ≈64% · Vision ≈66% · raw floor ≈47%** (§4.9, 2026-07-18 — recompute done: Web/Runtime spine folded in — HTTP client #2 + FS #5 + Uri + Unicode #6 + sessions #3). Weights 35 SYN(≈81%) / 40 FN(≈43%, still the drag but +6pp this span) / 25 RT(≈75%) on 824 rows. The ONE big lever remains the FN stdlib leg; next FN blockers = XML/streams/intl/SPL-heaps/mb-tail. |
...
> Detected in the post-audit gap session. Current position: **parity ≈64% · vision ≈66% ·
> raw floor ≈47%** (§4.9 recompute, 2026-07-18). Vision = 0.70×parity + 0.30×programme (§11.1, ratified).

...
| | |
|---|---|
| **Date / HEAD** | **2026-07-16 — FULL REOPEN AUDIT COMPLETE; the work order is §0.2 AUDIT BUILD QUEUE (25 slices, Tier 1 = security).** The row below this table's `Next` cell is the pre-audit perf-session record — historical (sqlbuild retired by DEC-208). *(pre-audit record:)* 2026-07-12 (session 5). **🏆 FULL SWEEP CLOSED — ALL 21 micros ≥ 1.0× vs fresh php:8.5-cli+JIT** (3× best-of-7 protocol medians, re-adjudicated + ratcheted): trycatch 33.4 · objalloc 9.1 · match 7.1 · **hofpipe 6.47** · floatarith 4.0 · methodcall 2.9 · interp 2.75 · webish 2.29 · strbuild 2.17 · closurecall 2.08 · **forin 2.05** · stringconcat 2.04 · fibrec 1.79 · enum 1.66 · **listappend 1.65** · listindex 1.61 · intadd 1.46 · mapget 1.10 · **mapinsert 1.08** · floatloop 1.00 · floatmul 1.00 (PARITY, ruled). Session-5 verticals: ACL list builders + reseed peephole (arena-cliff fix), AMB map builders (inline overwrite+insert), FnCap1 capturing closures + HOF loops (map/count), forin pointer-walk + mutation guard. Prior: session-4 flip phase (original 17), plan/spec consolidation. |
| **Completion** | **PHP-parity ≈62% · Vision ≈64% · raw row-floor ≈42%** — §4.8 recompute (2026-07-16, fable run): the TOP-20 #1 blocker FELL (all 10 FN-DB rows → 9 C + 1 P via the DEC-208 Core.Db execution wave: 3 drivers, typed hydration, streaming, transactions+retry, Secret, SQL-injection lint) + mail()/syslog rows COVERED (Core.Mail/Core.Log — DEF-031 header injection now UNREPRESENTABLE, better than PHP). Weights 35 SYN / 40 FN / 25 RT on 824 verdict rows (665 net). Next blockers: HTTP client (#2), sessions (#3), FS/streams (#5) — the Web/Runtime pillar packs. |
...
| **Next** | **SUPERSEDED 2026-07-16 — the work order is §0.2 AUDIT BUILD QUEUE.** sqlbuild as a Core fixture is RETIRED (DEC-208 removed the builder); the bytecode-inlining + scalar-replacement blueprint below is PARKED as a general JIT lever (revisit under DEC-266 if the dbwork/jsonround anatomies point at frame/dispatch overhead). Historical text kept verbatim: **Ω-1 gate levers: 6 of 8 W-slices SHIPPED** (`b59d9b0` general List.append clone · `021317c` native bridge2 join/contains/splitOnce/drop + str Eq/Ne · `8c1deb7` handle args/returns + builder-method Inst returns + entry str/list return materialization · `a5cac4a` fixpoint un-poisoning (pre-pass no longer stamps ctor-arg params Int; Int⊔handle join REVERTED as union-unsound) + `JitCache.redos` observability · `e4039b5` Bool consts + `Conversion.toString` · `c8cf1dc` handle-LIST/map instance fields). **SQLBUILD COMPILES END TO END (W7 `4dc7195` + W9/S8 `7be39e8` + clone pass-throughs): all widening slices DONE.** W7 = union Dyn cells (2-word register pair, declared-union seed `Function::dyn_params` breaks the fixpoint deadlock). W9 = (a) borrowed str/list ARGS clone at the call boundary (`rt_u_clone_value`; FLAT and runtime-borrowed-slot words pass through un-cloned — pinned + release-no-op), (b) Return FRAME TEARDOWN (owned below-cells released; Borrowed-Inst transfer census kept exact), (c) DEFERRED catch-pad seeding (PushHandler with unknown thrown class keeps walking — 3rd fixpoint-deadlock fix of the run). S8 = prelude JoinClause FLATTENED (14 fields, wide two-slot). Delivery test = the full builder chain, hits>0 + byte-identity; oracle 1967/1967; gate 21 WIN / 0 flips. **MEASURED (pinned, 2026-07-12): sqlbuild JIT 868ms vs VM 1869ms (2.15x over VM, hits=2 redos=0) but ~0.3x vs php — LOSS ANATOMY (ablation): the builder chain alone = 297ms/100k (~= php's ENTIRE runtime); toQuery + sql()/params() reads = ~5.7us/iter = 66% of total — the qualify*/String machinery (splitOnce/contains/join through the BOXING native2 bridge, ~15-20 calls/iter) + the 2 clone-returns.** **L1 FALSIFIED (2026-07-12, measured then REVERTED): arena fast paths for contains/splitOnce/join were built, VERIFIED HIT (48 contains + 18 join per 6 iters) and produced ZERO time delta — the native2 boxing is NOT the bottleneck. CORRECTED ANATOMY: toQuery's ~5.7us/iter is the qualify PIPELINE's accumulated machinery — per iteration ~6 boxed append-clones (the `out = List.append(out, q)` STR-list accumulator loops, each a full list_values->Vec->alloc), 3 List.drop bridge materializations, ~7 call frames each with borrowed-arg clones + `?` Result-enum wrapping. L2a SHIPPED `e38cd63` (UB_TAG_ACLS str-word records, zero-clone accumulator pushes, delivery test green, gate 21 WIN / 0 flips) — BUT the full sqlbuild number did NOT move (~0.9s pinned band): the real prelude's `?`-wrapped `qualify` most likely desugars ops between the append and its SetLocal so `accumulator_site` misses the shape (my plain-qualify delivery test HITS it). **L2a-completion slice SHIPPED (2026-07-12): dying-elem move + accumulator_site soundness + pool fail-open.** (1) `movable_dying_elem` (analyze.rs): the qualify-loop shape `out = List.append(out, q)` where q is a NON-PARAM single-use local pushed by the immediately-preceding GetLocal — the str-ACC arms consume the word and VOID the source slot (site-local by design: the blanket single-use widening regressed 10 tests + sqlbuild to VM speed and was reverted). (2) **PRE-EXISTING LEAK KILLED — accumulator_site positional hole:** `s = s + A + B` matched as an acc site at the outer Concat (the walk-back skipped the inner Concat, which pops-2/pushes-1 back to the same boundary depth and REPLACES the cell), so the emit elided SetLocal's old-release and the record leaked — one per statement, silently draining the 16-record pool for the WHOLE RUN (HEAD masked it because rt_u_acc_append falls back to plain concat on exhaustion; the new ACLS helper was fail-closed → redo storm → that's how it surfaced). Fix: reject any intermediate boundary depth ≤ lhs_index+1 in the walk-back. (3) Pool exhaustion now FAIL-OPEN in both list-builder twins (fallback = rt_u_list_append_clone, same MOVE semantics — correct bytes at degraded speed, never a code-5 storm; mirrors the str twin). Delivery test passes (hits>0, redos=0); full oracle green; clippy both configs + fmt + no-default clean; microbench-gate 21 WIN / 0 flips (strbuild 2.11 held). sqlbuild gate-noise ratio 0.268 (NOT yet snapshotted — protocol measure comes at the ratchet step). **CHAIN-ACCUMULATOR SHIPPED (same session): `accumulator_chain` (analyze.rs) generalizes the acc peephole to `s = s + A + B + …` left-spines — every link appends in place on the record (first link consumes the borrow, mid links carry the record, last link fuses the store; Int rhs renders via the interpolation decimal renderer; the span is validated whole so every position agrees — partial fusion structurally impossible). toQuery no longer re-boxes text per statement: sqlbuild 868→720ms/100k pinned (~17%); delivery test `phg_run_hook_hits_the_jit_on_the_chain_accumulator`; gate 21 WIN/0 flips (strbuild 2.15, webish 2.40 held). REMAINING sqlbuild anatomy: builder-step instance churn — each fluent step clones ~11 borrowed `this.field` args at the ctor boundary (W9a) + allocs/frees a wide two-slot instance. **Rc-SHARING SLICE SHIPPED (same session): boxed-handle clones are O(1) Rc bumps** — rt_u_clone_value (reprs 3/4/5), rt_u_native2 to_value (list args), rt_u_list_append_clone (boxed lhs via Rc::make_mut — in-place when unique, ONE copy when shared) all take the untagged-boxed fast path (`ub_is_untagged`); phorj value semantics already share via Rc+COW (every JIT in-place mutator is get_mut-guarded fail-closed), so this is the VM's own discipline = php's refcount equivalence, zero new machinery. Chain-only ablation 330→275ms/100k; full pipeline flat at ~720ms (counter anatomy per iter: 61 allocs, 29 clones now all Rc — clone_deep=0, 17 native2, 14 concat). ⚠ measurement gotcha: floatmul/floatloop sit at ratio-1.0 PARITY baselines — ±2% box noise flips them; A/B (7 interleaved pinned rounds, rc vs base binaries) proved the rc build NOT slower (median 5518 vs 5558) and a quiet-box rerun PASSed 21 WIN/0 flips. NEXT: **L2b blueprint (implement in a fresh-enough context; tree clean at 1fa1ff4):** (1) analyze CallMethod arm: record the RECEIVER'S ownership into sig slot 0 — `Kind::Inst(c, recv_own)` instead of `Kind::Unknown` (today this_inst overrides slot 0 with Borrowed unconditionally); the sig merge must meet ownership Borrowed-unless-all-Owned (check join_unknown_bottom's Inst arm — differing ownership must MEET to Borrowed, not decline). (2) this_inst: inject Inst(c, Owned) for `this` only when the merged sig slot 0 proved all-sites-Owned. (3) emit CallMethod: when the callee's this is Owned, the call site TRANSFERS the receiver word (no caller release of the dying temp — check who releases it today); mixed sites are impossible by (1). (4) field-TAKE inside an owned-this method: GetField(f) may TAKE the field word only when the method has exactly ONE `GetLocal(0); GetField(f)` pair for f, NO SetField on this, and this never escapes (not an arg, not returned) — an accumulator_site-style static scan; on take, write a BORROWED-SLOT sentinel (UB_TAG_SLOT, no OWNED bit — release no-ops) into the instance slot so the method-end husk release ladder cannot double-free the taken field (same discipline as the dying-temp GetField exclude, but surviving until teardown). (5) method-end: release the husk (taken fields are sentinel no-ops, untaken release normally). **L2b SHIPPED (layers 1+2 + prelude refinement):** transfer spine (a5cbdbc) + field-TAKE — `owned_this_taken_fields` (analyze.rs, shared by both walks): in an owned-this method with no loop/SetField/alias, a single-read `this.field` MOVES the word out (poison sentinel `UB_TAG_SLOT|UB_IDX_MASK` into the husk slot: teardown release no-ops, drift reads fault to code-5); strays (the `this.next(…)` receiver) allowed only when every method name resolving on our class has a GetLocal(0)-free body. ⚠ KEY FINDING: `next()` read this.tableName/tableAlias → its GetLocal(0) disabled ALL takes in its callers — fixed by threading both as params (prelude, S8-precedent; 6 call sites). Measured pinned: sqlbuild full 720→650ms/100k, chain-only 275→230ms (session cumulative 868→650). ⚠ box UNUSABLE for knife-edge gate verification at session end (load 5-6, TeamViewer active): floatloop/floatmul (parity-1.0 baselines) flip randomly across runs — one 7-round A/B (head vs l2b) medians 8479 vs 8774 (±15% intra-run variance, INCONCLUSIVE lean-noise). MUST re-run microbench-gate on a QUIET box before push; if floatloop still flips quiet, it's a ~2-3% code-layout effect on a parity-knife-edge micro → surface to developer (rebaseline is an adjudication, not self-ruled). **L2b REFINEMENT SHIPPED (35df29c):** take-at-LAST-read (max-ip, forward-only-branch proof) + per-field loop-span exclusion + transitive escape-read sets — counter-verified clone 29→18.5/iter, alloc 61→50/iter across L2b. **W10 (borrowed read-only params) FALSIFIED and reverted uncommitted** — trial-analyze marking is sound but net-negative: `new C(...)` is a CALL to a synthesized ctor, so borrowed forwards W9a-clone at that boundary (owned moves were free), and `return param` adds the borrowed-return clone; wall-clock regressed 0.65→0.72. Any retry must be CLONE-DELTA-AWARE (count borrow-clone sites in the trial vs baseline; accept only non-increasing). Detail in [[w7-union-dyn-cells]]. **STRATEGIC READ for ≥1.0×:** the remaining gap (650 vs 297ms) is DIFFUSE (18 Rc-clones + 50 allocs + 17 native2 + 7 call frames + 7 wide MakeInstances per iter) — single-helper levers are ~5-10% each; the structural lever is BYTECODE INLINING of single-call-site private methods (splice callee ops pre-analyze: kills the 7 frames + arg shuffling) followed by SCALAR REPLACEMENT of the dead-immediately intermediate builder instances (the fluent chain becomes pure register writes — the lever php structurally CANNOT match). That is a fresh-context major slice. **DEC-202 SHIPPED (bea7f61):** E-RESERVED-NAME now also rejects class-position names colliding with ~100 always-loaded PHP builtin classes (case-insensitive; foreign `declare class` binds EXEMPT — they bind to the builtin, the interop mechanism; free functions legal, aliases erase). ⚠ gotcha caught live: the interop golden test rejected examples/interop/classes.phg's `declare class DateTimeImmutable` before the foreign exemption — extern binds MUST be exempt from redeclare-class gates. Register row marked SHIPPED. **QUIET-BOX GATE VERDICT (2026-07-13): PASS 21 WIN / 0 flips** (floatmul 1.040, floatloop 1.017) — the flip saga is CLOSED: two 7-round interleaved A/Bs proved the phg side unchanged (medians 3873 vs 3924, crossings both ways); the FAILs were php-docker-side per-run variance around the EXACT-1.0 parity baselines. Pre-push state fully green (oracle + clippy both + fmt + no-default + gate). **PENDING gate-infra question for the developer (ratchet policy = BAR ruling, not self-ruled):** parity-1.0 baselines (floatmul/floatloop) false-FAIL on ±5% php-side swings — options: (a) epsilon band on the flip test (e.g. flip only < 0.95), (b) more MICROBENCH_RUNS for baseline-1.0 micros, (c) re-emit those baselines above 1.0 after a genuine win. **BYTECODE-INLINING BLUEPRINT (the sqlbuild ≥1.0 structural lever — execute in a FRESH context):** (1) Inline eligibility: a `Call`/`CallMethod` target with exactly ONE static call site in the graph, body ≤ N ops (start 64), no `PushHandler`, no recursion (target ∉ current inline stack), not the entry. (2) Mechanics: SPLICE the callee's ops into the caller pre-analyze (a bytecode-to-bytecode pass BEFORE resolve_unboxed_graph): remap the callee's slots to fresh caller frame slots (params = the arg expressions' stack results via SetLocal into the new slots), rewrite its `Return` to a Jump-to-continuation leaving the value on the stack, renumber jump targets. Run on a CLONED program (the VM's copy stays pristine — reified-operands invariant 6 untouched since only the JIT's compile input changes). (3) Iterate to fixpoint (whereGt inlines next; toQuery inlines qualifyAll…), cap total growth ×4. (4) SCALAR REPLACEMENT (second slice, after inlining measures): a MakeInstance whose result is used ONLY by GetField/GetLocal-borrow in the same (post-inline) function and dies there = replace the instance with its field slots (the fluent chain's intermediate builders become pure register writes — the lever php cannot match). (5) Measure per slice: pinned sqlbuild 100k + counters (frames should drop 7→~1/iter) + full gate. Files: new src/jit/inline.rs + a hook in resolve_unboxed_graph's entry; analyze/emit untouched (they see plain bytecode). NEXT: execute the inlining slice (fresh context) → protocol-measure → scalar replacement → sqlbuild ≥1.0 ratchet → Ω-1 remainder (Core.Db execution + HTTP + sessions = the §4.7-projected parity jump to ≈65-66%). Remaining ruled-unbuilt: DEC-203 (using+Closable), DEC-204 (Runtime.onShutdown, with Ω-2), DEC-205 (cycle collector→Weak), DEC-206 (DateTime gate). (DEC-214 empty collections — part-1 capability + part-2 `[]`-rejection — SHIPPED 2026-07-14, superseding DEC-201.) THEN (L2b) builder-step field-TRANSFER peephole (a dying temp receiver's fields move into the next instance instead of clone+release — kills the chain's ~3us too); (L3) refcounted handles (representation slice, V3b-adjacent, the php-COW-parity end state). Then protocol-measure sqlbuild, ratchet >= 1.0, THEN Ω-1 continues (Core.Db rusqlite, HTTP client, sessions). Deferred Dyn shapes (fail-closed declines, widen on demand): lambda union params, float-const+Call graphs (the v1 leaf-only float gate), MakeList-mixed literals (`[]`+append covers the prelude), Dyn re-assignment, map-kind borrowed args (no clone repr). |
...
> Detected in the post-audit gap session. Current position: **parity ≈62% · vision ≈64% ·
> raw floor ≈42%** (§4.8 recompute). Vision = 0.70×parity + 0.30×programme (§11.1, ratified).
...
**The ordered missing ledger (everything between ≈64% and 100%):**

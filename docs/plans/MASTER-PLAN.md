...
| | |
|---|---|
| **Date / HEAD** | 2026-07-12. Ω-8 verticals: trycatch NATIVE throw/catch (**33.4×**) + fully-inline mixed interpolation (**webish 2.2× · interp 2.5×**) + ACC-record accumulator (**strbuild 2.27×**) — map now **14 WINs / 17 micros** (remaining: intadd 0.68 checked · mapget 0.82 · listindex 0.99). Prior: 2026-07-11 plan/spec consolidation (perf-wave / di-attributes / web-spine / cli-name-sync / finishing-wave folded into this file + UNIFIED-SPEC + KNOWN_ISSUES; ROADMAP→thin pointer). |
...
| **Next** | Ω-8 flip-seq: **task 9 — checked-add range-proof elision** (the measured single root cause of ALL THREE remaining losses: intadd 0.68 · mapget 0.88 · listindex 0.97) → fundamentals micro sweep → representation slice (V3b + Rc cycle-leak §15 fork) → perf register + G-8 recompute → Ω-0…Ω-9. |

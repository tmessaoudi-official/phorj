...
> maps 67.1× behind php, and a per-component profile proved the VM dispatch floor alone is ~7-12× php's total
> and a boxed-JIT-via-`rt_` string path is ②-redux → a clean WIN on strings/arrays needs reimplementing php's
> 20yr-tuned engine natively in the JIT (unreachable at reasonable cost). **⚖️ MANDATE REFINED (dev, ask-human,
> §15 — supersedes "better than php per-feature or garbage"): phorj WINS where structurally possible (numeric /
> recursion / control-flow — ALREADY WON via the unboxed JIT: fibrec 1.7-2.9×, intadd #[Unchecked] 2×) and
> MATCHES-NOT-BEATS php on 20-yr-tuned string/array/collection categories.** **NEXT = ③ WEB SPINE** (Wave D:
> UA-L2 → SQL DBAL [SQLite→PG→MySQL, sync] → HTTP → sessions; rusqlite+rustls dep APPROVED, feature-gated,
> spine-quarantined; large new domain → FRESH context; first slice = SQLite DBAL, see wave3-4 drafts). Full
> evidence + the reverted value-repr/rebuild scoping = `perf-wave.plan.md` Decisions Log 2026-07-10.

| | |
|---|---|
| **Date / HEAD** | **2026-07-10 (latest · WAVE C STRING.FORMAT COMPLETE — 3b `%e`/`%E` + 3c `%g`/`%G` + 4a `%s`-precision + 4b `%N$`-positional + `%f`/-0.0 fix + `text.rs` M-Decomp all shipped; HEAD `af3aad3`. Full set `%s %d %f %e %E %g %G %x %X %o %b %%` + flags/width/precision/positional; `%g` verified by a 341k Rust-vs-php-8.5.8 sweep; slice-4 semantics developer-ruled STRICT (Invariant 15); remaining minor = `%c` + radix precision. ⭐ **NEXT-SESSION ORDER LOCKED (dev-ruled 2026-07-10, just say "continue"): ~~① RECOMPUTE parity/vision %~~ ✅ DONE 2026-07-10 (HEAD `af3aad3`, `M-gap-matrix §4.6` + `§11.4`): formal HEAD verdict-scan → exactly 2 rows moved (FN-STR sprintf ×4, RT-007 JIT GP→P; SYN-118 attrs improved but stays PARTIAL — 2/7 targets, no reflection) → **parity ≈60% · vision ≈62% · floor ≈41%** (chain 58%→59%→60%). Finding: the marathon (07-04→10, 203 commits) moved parity +1 (59→60); full span since ccb2403 (07-01), +2 (58→60) — small because it was perf+polish, the only breadth movers being crypto (§11.2) + sprintf; the DB/HTTP/sessions drag is untouched, which is exactly why ②/③ are next. → ② BOXED-VALUE JIT (the #1 perf lever, flips the ~11 VM-only LOSSes; big multi-session) → ③ WEB SPINE (Wave D: SQL DBAL → HTTP → sessions, the #1 parity drag). Detail in `perf-wave.plan.md` Decisions Log 2026-07-10.** (Deferred forks, NOT in this order: #7 attribute v2 + L1 runtime reflection, or slice 4.) — MARATHON RESTART (dev: "maximum autonomy, lots of features perf AND language").** Scope + full both-track queue RULED via ask-human (see `perf-wave.plan.md` Decisions Log 2026-07-10). **PERF STRATEGY RESOLVED (profiling-driven, developer-approved):** the "JIT #3 first" premise was FALSIFIED — a counting-allocator profile [Verified] proved methodcall/objalloc/enum are DISPATCH-CORE-bound, not alloc-bound; the cheap levers (inline-cache/Rc<str>/inline-fields) CANNOT flip them to WIN. **SHIPPED `4f482e9` (gate-green 1870/1870, php-8.5.8):** a monomorphic inline method cache (mirrors `field_caches`; methodcall 2.0→0.0 allocs/call, objalloc 5.0→3.0, byte-identical) — BANKED as an alloc reduction, explicitly FLAGGED NOT-a-WIN (still ~0.04× vs php+JIT). **RULING: the boxed-value JIT is now the evidence-proven PRIMARY perf lever** (fibrec, the sole JIT-eligible micro, is the sole WIN) — queued as the dedicated big multi-session PERF push. **LANGUAGE WAVE (developer-chosen for feature velocity):** **DI v1 SLICE 1 SHIPPED `c2a73ef`** (ctor autowiring, pre-check `desugar_di` → plain construction, synthesized `phorjInject<T>()` factories, default-SHARED diamond, single-impl interface auto-bind). **DI IMPORT-DISCIPLINE RETROFIT + SLICE 2 SHIPPED (2026-07-10, gate-green 1884/1884, byte-identical, unpushed):** slice 1 shipped `#[Injectable]`/`inject` AMBIENT — the developer flagged it (ask-human, §7) as the #1 recurring "in the wind" design error. FIXED — DI now lives in `Core.DI` under the SAME injected-Core discipline as `Core.Http`: bare via member-import (`import Core.DI.Injectable;`/`import Core.DI.inject;`) OR qualified (`#[DI.Injectable]`/`DI.inject<T>()`) via `import Core.DI;`; un-imported → `E-INJECTED-TYPE-BARE`/new `E-DI-NO-IMPORT`. `inject` UN-KEYWORDED (freed identifier). **Slice 2** = annotation-driven `inject()` (target from a typed `var`/`return`/lambda-ret; identical factory → byte-identical). STANDING RULE ratified: every future symbol import-gated, nothing ambient ([[nothing-in-the-wind-import-discipline]]). **DI v1 SLICES 1–4 NOW COMPLETE (gate-green 1900, byte-identical; commits `c2a73ef` `b657e26` `7e42fa8` `94a121c` `8390441` `4478fc6`, unpushed):** (3) field injection (fold injectable-typed no-init field → promoted ctor param, "synth-ctor"); (4a) `#[Provides]` static factories (precedence over new/single-impl); (4b) `#[Transient]` fresh-per-use via a LET-FLOAT codegen (`Built` tree, shared hoisted + transient inlined, byte-identical-PHP regression-guarded). Detail/log `di-attributes.plan.md` §7. **NEXT = DI v2** (qualifiers/generics/`#[Singleton]`/L1 runtime reflection §3). Then String.format 3+4 → trait → web spine. Detail SSOT = `perf-wave.plan.md` Decisions Log (NOT duplicated here). — prior: 2026-07-09 (pushed · **code through `451fb89`** = attribute foundation; `ce0ee7a` = DI spec; the SSOT/docs commits that follow can't cite their own sha) · **THIS CURSOR IS THE SSOT — read it first.** The detail logs `perf-wave.plan.md` (perf execution log) + `di-attributes.plan.md` (DI/attribute design spec) are REFERENCED here, not duplicated (no-duplication rule). **SHIPPED this session (all gate-green; dev pushes):** _(perf)_ overflow-model RULED = KEEP checked-default + `#[UncheckedOverflow]` opt-in (dev proposed inverting → challenged+kept, reverses ruled-IMMOVABLE `overflow-checked-by-default`); floatmul = **RESOLVED PARITY, NOT a flag**; checked-codegen tightening **PROBED→ABANDONED** [Verified VCode: Cranelift 0.133 can't emit non-trapping `add;jo` → default checked int unbeatable vs php, recover via `#[UncheckedOverflow]`]. _(language — user-attribute FOUNDATION, complete + usable)_ `#[Unchecked]`→**`#[UncheckedOverflow]`** under `Core.Runtime.Integer` two-mode (`ec219dc`/`09c680c`); **attributes parse on classes** (2a `d8d956e`); **`#[Attribute]` marker declares a user attribute** (2b-1 `92d9761`); **user attributes USABLE** — `#[Tag(...)]` on class/fn, COMPILE-TIME arg-count+type checked, byte-identical (2b-3 `bf05648` + 2b-3b `451fb89`); formatter class-attr fix. _(design)_ **DI + attribute-reflection SPEC captured** → `di-attributes.plan.md` (`ce0ee7a`). **NEXT (fresh session — the "very very big slice"):** **DI v1** = the one-session compile-time core per `di-attributes.plan.md` §1 (`#[Injectable]`; ctor+field autowire by type; immutable-safe field injection via synthesized construction-init; default-SHARED lifetime + `#[Transient]` opt-out; `inject<T>()` composition-root; single-impl interface auto; compile-time missing/ambiguous/cycle errors; byte-identical construction expansion, Inv-5). THEN attribute v2 layers + L1 runtime reflection (`di-attributes.plan.md` §3), and **PERF #3 (JIT object/enum/method — enum 0.01× · objalloc 0.34× · methodcall 0.05×) = the untouched perf frontier** (codegen-critical, fresh-context). **HONEST STATUS:** perf — JIT compute core WIN/match 3/15 micros, LOSE 3-100× on ~11 VM-only cats (=#3), G-8 met for core UNMET for breadth. language — attribute foundation complete+usable; DI designed, UNBUILT. **PARITY/VISION % [Speculative — full 824-row recompute (§11) DUE at the next milestone close]: ≈60% parity / ≈62% vision** (attribute foundation nudged vision; a globally "full rich complete phorj" now explicitly includes the DI/reflection/ORM/routing framework stack roadmapped in `di-attributes.plan.md` — a large future vision block). prior: **`#[Unchecked]` SHIPPED (`64ddf17`, gate-green, unpushed) → intadd LOSS→WIN** (opt-in wrapping int arith via `import Core.Unchecked` + attribute; JIT drops the overflow guard; [Verified: interleaved fresh docker php:8.5+JIT — intadd 3.23M vs php 6.41M ns = **1.99× WIN, 8/8, checksums identical**]; §14 LADDER `E-TRANSPILE-UNCHECKED`). Developer-adjudicated interactively (fault stays fail-fast, `#[Unchecked]` is the escape hatch; `Math.try*(): int?` recovery = follow-up). Compute-core: fibrec WIN · intadd WIN(unchecked) · floatmul 🚩FLAGGED(float-dep-bound). prior: **RANGE-ANALYSIS SHIPPED (`21465d8` code + `b5a1c9c` docs, gate-green, unpushed) — sound induction-counter overflow-guard drop; asm-verified the guard dropped (floatmul counter → plain `leaq`+`jmp`), but floatmul STAYS PARITY → 🚩FLAGGED (float-dependency-chain-bound, counter was off the critical path; the plan's "guard=residual" premise was WRONG; irreducible w/o FP-reassoc=Inv-1-forbidden → PENDING dev decision, perf-wave §Scoreboard, NOT self-ruled).** prior: floatmul PARITY (`5112967` dual-space ivars/fvars float value model — 4.5× LOSS→parity; perf detail in `perf-wave.plan.md`). **BAR = WIN-OR-FLAG** (beat php everywhere by any method; unbeatable→🚩FLAG as PENDING-DECISION, never self-rule). **Autonomy:** park §14/§15 forks + move on, 5-round advisor cap→park, commit each green slice, NEVER push. **Ordering:** perf is priority (sequence in `perf-wave.plan.md`: range-analysis→opt-in `unchecked`→tiering→Tier-2 breadth) — **when perf PARKS/forks, pivot to the language/sugar queue in "Next up" below**, never idle. Setup done+pushed (autonomous-3c sentinel + allow-list; deny/firewall intact, global layer). · **prior 2026-07-06: JIT MARATHON STARTED — execution order LOCKED = Option A (boxed-`Value`-via-kernels first, unboxing last; §Decisions-Log 2026-07-06). CODEGEN SLICE 1(a) SHIPPED, gate-green, unpushed:** the Cranelift JIT substrate — `cranelift` 0.133 behind the non-default `jit` feature (deps verified building on the 1.96.0 pin), the **unsafe island landed** (`#![forbid(unsafe_code)]`→`#![deny(unsafe_code)]` on both crate roots + the sole `#![allow]` in `src/jit/mod.rs`, CI `unsafe-island`-enforced), and `src/jit::compile_and_run` lowers a **default-deny int-arith leaf subset** (`Const`(int)/`GetLocal`/`AddI`/`SubI`/`MulI`/`DivI`/`RemI`/`Return`) to native code via `finalize→transmute→call`, arithmetic dispatched through the single-sourced `value.rs` kernels so overflow/div-zero faults are **byte-identical to the VM by construction** (Inv-4). 4 tests (`--features jit`): value≡VM-oracle, overflow+div-zero kernel-string parity, non-int default-denied; NEW CI `jit` job runs `-p phorj --features jit` (the `--workspace` gate never compiles the feature — closes that false-green). **NOT wired into `phg run`** (commit (b): wiring behind the eligibility predicate + control-flow branches/loops for fib + a differential example that provably hits the JIT). **No perf claimed** (unwired/unmeasured; the ~3× spike is a hypothesis for the wired path). **⚠ Full gate is now `--workspace` PLUS `-p phorj --features jit`.** Also shipped: **leak fix `c780540`** (`JITModule` has no `Drop` → `free_memory()` not `drop`, verified from cranelift source). **1(b) design FULLY LOCKED (perf-wave Decisions Log 2026-07-06):** native→native calls (Cranelift relocations, recursive fib JITs) + eager compile-all-eligible (no `--lazy` flag; hot-count = JIT-3) + program-lifetime module + a **memory operand stack** (sidesteps stack-VM→SSA phi; byte-identity by construction). VM seams to mirror are captured in the plan's 1(b) build-notes. **1(b) is the large SPINE-SENSITIVE build → start in FRESH context** (standing rule; advisor byte-identity review before its commit). Sub-commits: (b1) codegen+control-flow (unwired, unit-tested) → (b2) native calls+recursion → (b3) eligibility + `phg run` wiring + JIT-hitting differential examples (loop + fib) + honest fib measurement. Then JIT-2 → JIT-3 → JIT-4 → JIT-5 → Stage-2 re-measure → Stage-3 sweep → Stage-4 gate GREEN. — prior: 2026-07-05 (night) · **PERF-WAVE near-term win SHIPPED: `phg serve` → the bytecode VM by default (3 commits `caabfc4`/`d96bf9a`/`492c737`, gate-green 1801, unpushed).** serve now compiles + runs each request's `respond(bytes):bytes` on the VM (new `Vm::run_entry` = call-by-name + return-value capture; shared loop extracted to `run_to_completion`, `run_main` a thin wrapper, differential-proven byte-identical) instead of the tree-walker — **byte-identical** (dual-backend tests in `tests/serve.rs`, single-threaded AND through the multi-worker pool, since serve is outside the differential) and **~2.3× faster end-to-end** (measured 17.1µs vs 39.6µs median/request, keep-alive, release binary; handler-compute gain larger — fixed socket I/O in both). `--tree-walker` selects the interpreter oracle (also required for an overloaded `respond`, which the VM path rejects). serve.rs stays backend-agnostic behind a `Send+Sync` `HandlerFactory`; each worker compiles its own program (a `BytecodeProgram` holds `Rc` → not `Send`), amortised. A serve/web file with no `main` (entry is `respond`) gets an inert `ast::synth_empty_main()`. **Still ~25× slower than php+JIT — the per-feature perf mandate (G-8) is UNMET until the Cranelift JIT (RULED, Step 4, docs/plans/perf-wave.plan.md); serve→VM was the ruled bounded near-term win.** NEXT: the Cranelift JIT (first gate = the dependency-policy amendment) — a multi-session marathon. — prior: **DEC-197 SLICES 1 + 2 SHIPPED (function member-imports, Core natives AND user packages) — gate-green (1788), unpushed.** **SLICE 2 (user-package functions, loader layer):** `import App.Text.banner;`→bare `banner(…)`, `as`-aliased, grouped; resolves in the loader (`build_function_imports` mirroring `build_type_imports` off the `defined`/`prov_fns` tables) → rewrites bare call AND value-position reference to the same mangled FQN a qualified `Text.banner(…)` call produces, so byte-identity is INHERITED from the proven qualified cross-package path (run≡runvm structural; PHP manually verified — the project differential `all_example_projects_match_between_backends` is run≡runvm-ONLY, a false-green trap advisor flagged). STRICT (member ≠ qualified sibling), `local > same-pkg fn > imported`; `E-VIS-PRIVATE` on a private fn, `E-IMPORT-CONFLICT` on a duplicate bound name (alias with `as`); function imports excluded from `user_import_map` (disjoint maps). Cross-layer ambiguity (same bare name from a user pkg AND a Core native) resolves user-fn-wins by the precedence chain — NOT an error (advisor-corrected; keep E-IMPORT-CONFLICT same-tier only). Multi-file example `project/function-imports/` (byte-identical run≡runvm≡php-8.5.8, incl. cross-package operand `addUp(1,2)+1` + first-class VALUE `var f = banner` — the value-position path advisor caught as a blind spot, fixed in `resolve_expr`) + README + 6 loader tests. Disclosed (KNOWN_ISSUES): loader is pre-scope → a local shadowing an imported fn name resolves to the import (same pre-existing limitation as same-package fns; not byte-identity). **String.format SLICES 1+2 SHIPPED (2026-07-05, gate-green 1798): PHP `%` sprintf, strict.** `Core.String.format(spec, [values])` — real native (`text_format` + gated PHP `__phorj_format` which DELEGATES `%d`/`%f` to real `sprintf` so the PHP leg is canonical; `%s`→`__phorj_str`). Slice 1 = `%s`/`%d`/`%%`; **slice 2 = flags `-`/`0`/`+`, width, and `%f` (`.precision`, default 6)** — `%f` uses Rust `format!` round-half-to-even, VERIFIED ≡ PHP sprintf (2.5→2, 3.5→4, sign-before-zeros `%05d`-of-`-42`→`-0042`, int→float, `%05s`), full oracle case table byte-identical run≡runvm≡php-8.5.8. STRICT (`%d`/`%f` wrong type → clean fault, fault-parity all 3 legs); heterogeneous value lists; qualified + DEC-197 bare import; shared `parse_format_directive` (renderer + compile-time gate agree). **decimal NOT formattable by `%f`/`%d`** (faults on all 3 legs — consistent; use `%s`). `guide/string-format.phg` + 10 checker tests + 6 `E-FORMAT-*` explains. **NEXT: slice 3** = `%x/%o/%b/%e/%g`; slice 4 = `%N$` positional + precision on `%s`/`%d` (each a byte-match-PHP-sprintf increment). **SYNTAX RULED DEC-199 (2026-07-05, SUPERSEDES DEC-198): PHP-style `%` sprintf, NOT `{}`.** Chain (all challenged): positional-literal-format is redundant with interpolation → format's only real job is a RUNTIME/dynamic spec → a runtime spec can't be statically checked in any syntax → `{}`'s only advantage (static checking) evaporates → `{}` would be pure PHP-divergence with no payoff; `%` is ALSO collision-free (the DEC-198 `{}`/`{expr}`-interpolation lexer blocker — verified `String.format("val {}",[3])`→parse error) and transpiles to literal `sprintf`. Phorj UPGRADE: render STRICTLY (type mismatch → clean fault, not PHP silent coercion). `{}` stays interpolation-only (interpolation specifiers = separate W5-1 decision). **BUILD (spine-sensitive):** a Rust `%`-sprintf renderer byte-identical to PHP `sprintf` (interp+VM match the transpiled `sprintf` leg); slice by conversion set (`%s`/`%d`/`%f`+width/precision/`-`/`0` first; `%x/o/b/e/g`/`%N$` later), each byte-verified run≡runvm≡php-8.5.8; dynamic spec supported, unsupported conversion → clean fault. — prior: **DEC-197 SLICE 1 SHIPPED (native function member-imports) — gate-green, unpushed.** The two-mode import discipline now extends to Core module FUNCTIONS: whole-module `import Core.Output;`→qualified `Output.printLine(x)` (unchanged) OR member `import Core.Output.printLine;`→bare `printLine(x)`; `import Core.String.trim as clean;` aliased; grouped `import Core.Math.{ abs, max };` canonicalizes to one/line; UFCS coexists. **STRICT** (member import does NOT enable a qualified sibling — falls out free: the checker's `import_map` never binds the module leaf for a member fn import → sibling `Output.print` stays E-UNKNOWN, exactly the intrinsic model). Build: new `src/checker/function_imports.rs` (`function_import_bindings` = single source of the binding set) → `fn_imports` map built in `collect` → `check_named_call` resolves a bare call AFTER user funcs (`local > user fn > imported native`), records a bare→qualified `Member` rewrite REUSING the original span (reified-operand-safe) via `ufcs_resolutions`; any omitted trailing default folded into the one rewrite. E-PKG-CASE carve-out GENERALIZED (Assert/Abort special-case → any camelCase member-import leaf is a value/function leaf, exempt; a function alias is camelCase = E-NAME-CASE); transpiler binds the member-fn module leaf locally (checker map stays clean/strict — the asymmetry is sound: checker gates before transpiler sees a sibling). `E-IMPORT-CONFLICT` on a duplicate bound name (alias with `as`). Example `guide/function-imports.phg` (byte-identical run≡runvm≡php-8.5.8, incl. NON-Math bare native as an arithmetic operand `length("hi")+1` — the decisive reified-operand path advisor flagged — + `parseFloat` default-fill fold verified in the PHP emit) + README + 11 checker tests; full gate **1782 green**. Disclosed (KNOWN_ISSUES): a non-callable local shadowing an imported fn name resolves to the import (not byte-identity; deferred fix threads full lexical scope — same as slice-2's harder loader-layer "local > imported"). **NEXT: DEC-197 SLICE 2 (user-package function member-imports — loader layer, `build_function_imports` mirroring `build_type_imports`), then String.format.** — prior: **DEC-196 COMPLETE (Q1–Q4) + FAULT/OUTPUT-PARITY sweep + WAVE C FOUNDATION** — all shipped this session, gate-green (php-8.5.8), developer pushed through `09453a7`; 2 commits ahead after (`6e9b748` split/characters, `e4c4802` Wave C foundation). **FAULT-PARITY + OUTPUT-PARITY passes (docs/research/fault-parity-pass-2026-07-05.md):** exit-status value-guard set consistent; FOUND+FIXED 2 real byte-identity breaks — `Conversion.truncate`/`round` out-of-range (→ fault, `09453a7`) + `String.split(s,"")` (→ fault + new `String.characters`, `6e9b748`); pow(0,neg)=disclosed UA-0.14 only. **WAVE C FOUNDATION SHIPPED (`e4c4802`):** call-arg expected-type threading — a list/map literal now threads a CONCRETE `List/Map<union>` param (`check_arg`→`thread_literal_expected`, `ty_has_param`-guarded; ⚠ `check_arg` is on EVERY call path — hit 2 hot-path regressions, both gate-caught+fixed+locked). **⛔ NEXT SESSION (fresh context): (1) DEC-197 import-model unification — SLICE 1 (natives) SHIPPED (see lead above); SLICE 2 (user-package function member-imports) REMAINS** — loader-layer `build_function_imports` mirroring `build_type_imports` (bind bare name → mangled FQN; `as` alias; grouped rides DEC-186; the loader is pre-scope so honoring `local > user fn > imported native` is the harder version of slice-1's shadowing gap — decide the layer deliberately). **DEC-197 GATES String.format** (format is a function → its import/call form is defined by DEC-197). **(2) then the `String.format(spec, args)` native + `{}`-spec grammar** (shared with unbuilt W5-1 interpolation specifiers) across all 5 backends. Then Wave D web spine. **Also still open (fresh context): the remaining ~50 lower-risk raw-builtin output-parity probes** (array/libm/hash/path/url). — prior: **DEC-196 CLEANUP PROGRAMME (2026-07-05 examples/conformance audit — §13.1).** **Q1 (examples/ layout + doc-name reconciliation) ✅ SHIPPED this session** — `git mv examples/fmt/`→`format/` + `bench/`→`benchmark/` (every ref updated + `examples.js` regen, 146 entries), `phg bench —`→`benchmark —` report header (+2 tests), dead-verb rustdoc sweep (`phg fmt/bench/disasm`→full verbs, module/file/fn names untouched), `web/core-http.phg` explicit `import Core.String;` (verified coexists with the Http prelude), `STABILITY.md` module names→real registry names (6 ruled + `Crypto`→`Cryptography`), `git rm docs/plans/wave0-remainder.plan.md`; full oracle gate **1748 green** (php-8.5.8), 141-example differential intact. Q4 (playground) shipped `b47fc72`. **Q2 [COMPILER, breaking] ✅ SHIPPED this session** — native renames `String.uppercase`→`upperCase` / `String.lowercase`→`lowerCase` (name-only; PHP emit unchanged) + charter regression denylist; full gate green. **Q3 ⛔ RE-OPENED — PENDING developer adjudication (§13.1 DEC-196 Q3):** the intrinsics-import build surfaced a source conflict on BARE vs QUALIFIED call syntax (DEC-196 "used bare" vs UNIFIED-SPEC "nothing in the wind" qualified + audit §1.11 developer instinct "bare reads inconsistent"). Invariant-15 fork → surfaced via AskUserQuestion, NOT self-ruled. Build is checker-only + 2 examples, ready the moment the syntax is ruled. **This session's shipped work = Q1 + Q2 (both green, committed).** — prior: **WAVE B — B-1 (types) + B-2a (Option combinators) SHIPPED.** **B-2a:** `Core.Option` combinators `map`/`andThen`/`filter` (higher-order natives) + `getOrElse` (eager default) + `ofNullable`/`toNullable` (`T?`↔`Option` bridge), in `src/native/option.rs`, UFCS-dispatched (`opt.map(f)` → `Option.map(opt,f)`), erasing to gated `__phorj_option_*` PHP helpers over the injected `Some`/`None` classes. Example `guide/option-combinators.phg` + 7 unit tests; full gate green. Invariant-7 PROVEN by differential (`some.getOrElse(0)+1`=6 byte-identical). Two justified in-slice extras: (a) FIXED a general pre-existing CRASH — a `new` inside a UFCS-relocated arg subtree (`xs.map(fn => new C())`, and `f(new X()) as T`) bypassed `unwrap_new` → `Expr::New` panic; `rewrite_ufcs::rexpr` now strips it (guarded by non-Option `guide/ufcs-construct-in-lambda.phg`); (b) widened `unify` so an `Optional(T)` param binds `T` from a non-null arg (`ofNullable(42)`; aligns with the existing assignability rule; only `ofNullable` has an Optional first-param → no UFCS ambiguity). **B-2b Result combinators SHIPPED (DEC-185, full 8-native set).** `Core.Result` natives (`src/native/result.rs`): `map`/`mapErr`/`andThen`/`orElse` (HigherOrder) + `getOrElse` (eager) + `toOption` (Result→Option bridge) + `isSuccess`/`isFailure`; UFCS-dispatched, gated `__phorj_result_*` PHP helpers (`isSuccess`/`isFailure` inline `instanceof`); `filter` deliberately omitted (no error to synthesize). Example `guide/result-combinators.phg` + 7 unit tests; full gate **1727 green**, byte-identical run≡runvm≡php-8.5.8; Invariant-7 proven (`getOrElse(0)+1`=2). Root-caused + FIXED a P0 byte-identity break (advisor-surfaced): `Result.toOption` without `import Core.Option` type-checked+ran but PHP-fataled (`Some` class missing) → new checker guard `E-RESULT-TOOPTION-NEEDS-OPTION` rejects it in lockstep (UFCS + qualified forms) + explain + 3 tests. **DISCLOSED (KNOWN_ISSUES, pre-existing, NOT B-2b): LSP `diagnostics_for` runs the raw checker (no prelude injection) → spurious unknown-type squiggles on ALL injected-type programs (Json/Option/Result/…); corrects "LSP DoD satisfied by construction" (true for natives, NOT injected types) → dedicated LSP slice.** **B-2c variant imports SHIPPED (DEC-186, `05cbd9b` parser + part-2 binding).** `import Core.Result.Success [as X];` / groups `import Core.Result.{ Success, Failure as X };` bind an injected enum's variants bare/aliased, usable in construction + `match` patterns; pre-check pass `resolve_variant_imports` rewrites them to the qualified form (reuses proven byte-identical machinery); collisions → `E-IMPORT-CONFLICT`, nonexistent variant → `E-IMPORT-UNKNOWN`, empty group → `E-IMPORT-GROUP-EMPTY`. Zero-payload pattern keeps the parens rule (`None()`). Example `guide/variant-imports.phg` + 3 parser + 5 checker tests; full gate **1735 green** byte-identical. `phg format` canonicalizes a group to one import/line (no group AST node). The earlier "zero-payload catch-all trap" was CORRECTED to the pre-existing zero-payload-needs-parens rule (not a new bug; the pass rewrites only the parens `Pattern::Variant`, bare identifiers stay catch-alls as before). **DEC-187 width-canonical `fmt` wrapping SHIPPED (2 commits: `2b2ac04` doc-IR conversion byte-identical + step 2 wrapping live).** AMENDED at session start from the original "expand-only" ruling to **WIDTH-CANONICAL** (drop Rule 1 "preserve author breaks" — developer-adjudicated; §13.1). New `src/fmt/doc.rs` (Wadler doc-IR: `Text`/`Line`/`SoftLine`/`Concat`/`Nest`/`Group` + `fits` + column renderer); `printer::expr()` builds a `Doc`, a flat wrapper keeps every non-wrapping context byte-identical (the hybrid seam — statements/comments/decl headers stay imperative). Statement values render at a 100-col budget: call/`new`/`parent` args, collection + map literals, `match` arms, and `.`/`?.` chains (≥2 links) break one element per line when they overflow, collapse when they fit; a gratuitously hand-broken short chain now collapses. **Interpolation holes NEVER break** (correctness — a newline would change the string value). Whole corpus reformatted (35 files) + dogfood strengthened to `fmt(src)==src` (folds UA-0.8); `examples/fmt/showcase.phg`+README; LSP formatting reuses `fmt::format` (both editors free). 8 doc-core + 4 behaviour unit tests; full gate green, byte-identical `run≡runvm≡php-8.5.8` across every reformatted example (141 differential). Deferred+disclosed (KNOWN_ISSUES): binary-op chains, decl param lists, class headers, control-flow conditions. **B-2d rich-error AUDIT DONE (`docs/research/b2d-rich-error-audit.md`):** reframes the slice — UA-1.8 fault-canonicalization is DONE **except** a small reachable VALUE-guard set (6C correction: only SIGNATURE guards `expects (types)` are checker-unreachable/skip; VALUE guards with `{}`/`cannot`/`found` — e.g. `Conversion.toString cannot convert {}` — type-check clean and fault at runtime → must be traced + canonicalized, NOT skipped; that set was marker-grep'd = a STARTING set, a definitive close needs a trace-based reachability pass over every native fault, not pattern-matching). Initially flagged 3 "latent divergences" (`List.chunk`/`Hash.hkdf`/`Conversion.toString`) but **RETRACTED same-day**: they all FAULT in PHP → behaviourally consistent (fault-parity = "PHP must fault too", text need NOT match — `agree_err` is run≡runvm-only, faults aren't byte-identity examples). DEC-195 guard-helpers = cosmetic, NOT built, developer to re-decide. The REAL hazard (Phorj-faults-but-PHP-SUCCEEDS) pass was NOT run. See audit banner. Established the verify-regime (differential is text-blind for `FaultKind`-classified faults; per-string agree_err/string-assertion required — aggregate-green is insufficient) + the helper-vs-raw-builtin lowering distinction. **NEXT: DEC-180 reclassification** (raw-builtin-divergent faults → Result/`T?`/guard-helper — §15 forks, developer-adjudicated, fresh context) → Wave C. — prior: **foundation slice B-1 SHIPPED (injected `Core.Option`/`Core.Result` TYPES).** DEC-182 *foundation only* — the two canonical types are now compiler-injected (gated on `import Core.Option;`/`import Core.Result;`), NOT the full DEC-182 (combinators + `T?`↔`Option` conversions = slice B-2, still pending). First *generic* injected enums; `T`/`E` checked as `Ty::Param` then erased downstream (verified: `erase_generics` runs after the inject chain). Mirrors `inject_rounding_mode_prelude`; variants qualified-only (`Option.Some`/`Result.Failure`, bare = `E-INJECTED-VARIANT-BARE`); user-declared same-name enum shadows + skips injection. Ships `examples/guide/core-option.phg` + `core-result.phg` (byte-identical run≡runvm≡php-8.5.8) + 6 checker tests; full gate **1710 green**. Wave A remains near-complete (call-arg threading → Wave C). **FOUND + DISCLOSED (KNOWN_ISSUES, not fixed — pre-existing, not from B-1):** two F-m reserved-name-guard gaps (enum *variant* names unguarded; PHP *builtin class names* like `ParseError`/`Error` not in the set) → run/runvm-succeed-but-PHP-fails byte-identity break; fix = a later F-m pass. **NEXT: slice B-2** — Option/Result combinators (`map`/`andThen`/`filter`/`getOrElse`) + `T?`↔`Option` conversions; GATING CHECK for B-2 = does `opt.map(fn)` method-syntax resolve to a module native on an injected-*enum* value (UFCS precedent is collection natives, not enums)? If not → §15 fork (static-only vs add enum methods), surface don't self-rule. — prior: **FORK-BACKLOG ADJUDICATION PASS COMPLETE + Wave A starting.** Prior marathon (`f8b8cd1`) pushed by developer. This session cleared ALL open §15 forks interactively (§13.1 / DEC-177…181): trait BLESSED, W3-5 blocker→Wave A, error model→honor-3-tier, editors→LSP-first-then-native, UA-1.8 shape. Only W4-10 XML deferred. Rulings merged into this plan + `C-decisions.md`; temp session plan removed (one-SSOT rule). **NOW BUILDING Wave A (Type-System Completion).** — prior-run note below: **AUTONOMOUS MARATHON — 18 green commits `b3bd402`→`7e5c389`, all full-oracle-gate verified (1661 tests, php-8.5.8), unpushed. Clean checkpoint: all CLEAR/unambiguous high-value work done; remaining items need the developer (design/§15/fresh-session) or are P2-with-investigation.** DONE this run: **M0** — examples.js determinism, mold, --help fixes (UA-0.2/0.3/0.4), 18 specs archived+repointed, UA-0.6 (E-STATIC-FIELD-VIA-INSTANCE diagnostic), UA-0.17 (2 ghost explain drops), UA-0.18 (Suspend doc), UA-0.14-partial (Core.String stale-ASCII fix + String.length/List.append disclosures), nextest restored · **M1** — all small/medium byte-identity fixes (UA-1.1 trim, 1.2 reverse, 1.3 pbkdf2, 1.4 hmac→bytes, 1.7 clamp) + UA-1.8-part1 (fault-string names) + 1.9 import example + 1.10 playground forbid · **M5** — editor refresh (VSCode 0.3.0 + PhpStorm no-build path confirmed working). Deferred w/ evidence: UA-0.1 (nextest was the real lever). Recorded PENDING/couplings: PhpStorm native plugin (scope), UA-1.6↔W3-5 (expected-type→literal threading — build once, unblock both). **NEXT (developer/fresh-context): UA-1.8 part-2 shapes (pick canonical format), UA-1.5 (→ retirement, NO bulk sed), M2 UA-L2 (prelude/loader unification — design pass, gates W3), M3 web spine (W3-5 §15 {}-grammar blocker first); P2 tail UA-0.5/0.7/0.9/0.10/0.11/0.13/0.15/0.16.** — original run note below. |
...
| **Current phase** | **PERF WAVE (2026-07-05, `docs/plans/perf-wave.plan.md`).** VM ceiling PROVEN (no bytecode-VM tuning closes the 9–154× php+JIT gap) → **Cranelift JIT RULED** as the only path (Step 4; first gate = the dependency-policy amendment — a multi-session marathon, NEXT). Shipped this session as the ruled bounded near-term win: **`phg serve` → the VM by default** (byte-identical, ~2.3× faster; `--tree-walker` for the interpreter; `caabfc4`/`d96bf9a`/`492c737`, gate-green 1801). Perf mandate (G-8) still UNMET pending the JIT. Prior phase: **DEC-196 CLEANUP MARATHON (2026-07-05, §13.1).** Q4 shipped (`b47fc72`); **Q1 + Q2 ✅ SHIPPED this session** (Q1 examples/ layout + doc reconciliation; Q2 native casing renames `uppercase`→`upperCase`/`lowercase`→`lowerCase`, name-only breaking + charter regression denylist — full oracle gate green). **Q3 ✅ SHIPPED this session** (§13.1 DEC-196 Q3): after surfacing a bare-vs-qualified source conflict (Invariant-15 fork → AskUserQuestion), developer ruled the TWO-MODE model (module→qualified `Assert.assert`, member→bare `panic`, grouped ok); built as `resolve_intrinsic_imports` (raw-program validate + qualified→bare normalize; backends unchanged, byte-identity preserved) + casing carve-out + `E-UNIMPORTED`/`E-IMPORT-UNKNOWN` + `phg explain` + 3-mode example + 12 tests; full gate green. **DEC-196 COMPLETE (Q1+Q2+Q3+Q4).** Each step: full oracle gate (php-8.5.8) + Invariant-9 example + commit; NEVER push. Prior: fork backlog cleared 2026-07-04 (§13.1/DEC-177…181); Wave A shipped; Wave B B-1/B-2a–d + DEC-187 fmt shipped. Keep THIS cursor current every working session (developer standing rule). |
...
| **Gate** | `source scripts/toolchain.env && PHORJ_REQUIRE_PHP=1 cargo test --workspace --features jit` + clippy + fmt + release build (oracle php path = the single editable knob in `scripts/toolchain.env`, currently `php-8.5.8`). **`--features jit` is required** — the JIT is not a default feature, so bare `cargo test --workspace` skips it. Pre-commit = fast Rust-only tier (`PHORJ_SKIP_PHP=1`, `nextest --features jit`, excludes 2 heavy sweeps → ~12s); pre-push = full suite + clippy(both) + full 8.5 oracle + microbench-gate. |

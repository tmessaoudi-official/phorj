...
- **W3-5 · `String.format` (sprintf family)** — design RULED (developer, 2026-07-03): lives on
  ...
  **✅ GRAMMAR RICHNESS RULED — DEC-198 (developer, 2026-07-05, after re-challenging `{}`-vs-`%`):**
  the `{}` grammar is the **FULL Rust-style mini-language** — positional `{}` + indexed `{0}` +
  `{:spec}` where spec = `[fill][align: <^>][sign][width][.precision][type: b/o/x/e/…]`, ONE grammar
  shared by `String.format` AND string interpolation (W5-1). `%`-style stays rejected: interpolation is
  already `{}` (a second grammar would be a PHP-style wart), and static typing makes `{}` checker-verified
  by arg type (no `%d/%s` footgun). PHP interop unaffected — the lifter rewrites `sprintf`, and transpile
  still EMITS `sprintf`/`number_format` under the hood. Import/call form defined by DEC-197 (it's a

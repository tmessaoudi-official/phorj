...
| DEC-519 | DEC-514 scoped L4c to the `nestedlist` / `namedtuplefield` pair, but four carried OWED micros are element-access-heavy loops over the same shape | RULED 2026-09-09 — **widen L4c's root-cause phase** to measure `fslines` / `queryparse` / `fsforeachline` / `strappend` as corroborating evidence before the hypothesis. AMENDS DEC-514's scope; its root-cause-first requirement is untouched. Lane order unchanged |

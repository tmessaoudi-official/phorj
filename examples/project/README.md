...
| `tag-package/` | **Template tags in a library package** (scout row 5h1) — `mark"…"` and `Loud"…"` resolve the CURRENT package's own tag function and tag type from sibling files, never `Main`'s same-named `mark` (see `tag-package/README.md`) |
| `ufcs-package/` | **UFCS on a library package's own functions** (DEC-527) — `name.shout()` resolves a function of the CURRENT package from a sibling file, and a `private` helper method-style from its own file; another file gets `E-VIS-PRIVATE`, another package gets an error (see `ufcs-package/README.md`) |
...
| `reflectdump/` | **The two `get_class()`-keyed helper tables under NAMESPACED emission** — the probe KNOWN_ISSUES §TRANSPILE-NS-REFLECT-TABLES asked for. A `package Acme` enum is dumped (`Debug.dump`) and a cross-package class is reflected (`Reflection.methods`/`interfaces`/`parents`) from `package Main`. Measured 2026-09-05: the reflect table was already namespace-safe; the enum-dump table was NOT — the PHP leg printed `Acme\Color_Green {}` where both native legs print `Acme\Color.Green` — fixed in the same change. The differential gates it on all three legs from now on |

...
mod graph_info;
pub(in crate::jit) use graph_info::*;
mod kinds;
pub(super) use kinds::*;
...
/// Kinds that cannot cross a call boundary as an argument (`Op::Call` + `Op::CallMethod`, one
/// predicate for both). [`Kind::Fn`] is deliberately absent and [`Kind::FnCap1`] present — see their
/// docs for why (DEC-445).
fn blocked_as_call_arg(k: &Kind) -> bool {
    k.is_handle() || *k == Kind::EnumInt || matches!(k, Kind::FnCap1(_))
}
        ...
        loop {
            ...
            match &code[ip] {
                ...
                Op::CallNative(id, 1) if unboxed_native_is_map_keys(*id) => {
                    admit_map_query1(&mut kinds, Kind::StrList(Own::Owned), "Map.keys")?;
                }
                Op::CallNative(id, 1) if unboxed_native_is_map_values(*id) => {
                    admit_map_query1(&mut kinds, Kind::IntList(Own::Owned), "Map.values")?;
                }
                Op::CallNative(id, 1) if unboxed_native_is_map_size(*id) => {
                    admit_map_query1(&mut kinds, Kind::Int, "Map.size")?;
                }
                ...
                // `MatchTagName` rides the same register shape; its name-uniqueness gate lives in
                // `collect_functions_unboxed` (single-desc names make tag-compare ≡ name-compare).
                Op::MatchTag(_) | Op::MatchTagName(_) => {
                    let k = kinds.pop().ok_or_else(|| {
                ...
                Op::Return => {
                    ...
                    let rk = match rk {
                        ...
                        // Same no-decode-repr argument as IntSet: a map-word list never escapes.
                        Kind::MapList(_) => {
                            return Err(JitError::Unsupported(

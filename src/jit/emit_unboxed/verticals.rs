...
//! the flat index reads, the map bucket probe, inline concat,
...
pub(super) fn arm_index_map(
    ...
    let mv_flat_up = b.ins().ishl_imm_s(mv, 1);
    let fused = b.ins().band(mv_flat_up, iv);
    let both = b.ins().band_imm_s(fused, UB_TAG_SLOT);
    ...
    let kslot_bit = b.ins().band_imm_s(iv, UB_TAG_SLOT);
    ...
    let aki = b.ins().band_imm_s(iv, UB_IDX_MASK);
    let akoff = b.ins().ishl_imm_s(aki, 6);
    ...
    let aridx = b.ins().band_imm_s(mv, UB_IDX_MASK);
    let aroff = b.ins().imul_imm_s(aridx, 24);
    ...
    let amask = b.ins().iadd_imm_s(atsize, -1);
    ...
    let abtoff = b.ins().ishl_imm_s(at, 4);
    ...
    let aempty = b.ins().icmp_imm_s(IntCC::Equal, aecanon, 0);
    ec.fault_if(b, aempty, 5); // genuine miss → canonical fault on the VM redo
    let at1 = b.ins().iadd_imm_s(at, 1);
    ...
    let ki = b.ins().band_imm_s(iv, UB_IDX_MASK);
    let koff = b.ins().ishl_imm_s(ki, 6);
    ...
    let lg_raw = b.ins().ushr_imm_s(mv, UB_MAP_LOG_SHIFT);
    ...
    let tsoff = b.ins().ishl_imm_s(cnt, 7); // table starts after the 2·cnt pair slots
    ...
    let empty = b.ins().icmp_imm_s(IntCC::Equal, bcanon, 0);
...
pub(super) fn arm_iter_ptr_init(
    ...
    let cbytes = b.ins().ishl_imm_s(cnt, 6);
...
pub(super) fn arm_set_index_map_local(
    ...
    let slot_bit = b.ins().band_imm_s(iv, UB_TAG_SLOT);
    ...
    let abase = b.ins().load(types::I64, ec.stable, ec.ctx, 40);
    let ridx = b.ins().band_imm_s(mv, UB_IDX_MASK);
    ...
    b.switch_to_block(next);
    let empty = b.ins().icmp_imm_s(IntCC::Equal, ecanon, 0);
    ...
    let cap = b.ins().load(types::I64, MemFlagsData::new(), prec, 16);
    let count1 = b.ins().iadd_imm_s(count, 1);
    let lf = b.ins().ishl_imm_s(count1, 1);
    ...
    let tbytes = b.ins().ishl_imm_s(tsize, 4);
    let rbytes = b.ins().ishl_imm_s(count1, 3);
    let need0 = b.ins().iadd(tbytes, rbytes);
    let need = b.ins().iadd_imm_s(need0, 16);
    ...
    b.ins().store(MemFlagsData::new(), vv, ebase, 24);
    let roff2 = b.ins().ishl_imm_s(count, 3);
...
pub(super) fn arm_str_len(
    ...
    let fast_blk = b.create_block();
    let slow_blk = b.create_block();
    let slot_bit = b.ins().band_imm_s(sv, UB_TAG_SLOT);
    if sk.is_owned_handle() {
        ...
        let acc_bit = b.ins().band_imm_s(sv, UB_TAG_ACC);
        b.ins().brif(acc_bit, fast_acc, &[], slow_blk, &[]);
        b.switch_to_block(fast_acc);
        let abase = b.ins().load(types::I64, ec.stable, ec.ctx, 40);
        let aidx = b.ins().band_imm_s(sv, UB_IDX_MASK);
        let aroff = b.ins().imul_imm_s(aidx, 24);
        let aprec = b.ins().iadd(abase, aroff);
        let alen = b.ins().load(types::I64, MemFlagsData::new(), aprec, 8);
    ...
    b.switch_to_block(fast_blk);
    let buf = b.ins().load(types::I64, ec.stable, ec.ctx, 0);
    let si = b.ins().band_imm_s(sv, UB_IDX_MASK);
    let soff = b.ins().ishl_imm_s(si, 6);
...
pub(super) fn emit_release(b: &mut FunctionBuilder, ec: &Ec, h: &UbHelperRefs, v: ClValue) {
    ...
    let is_borrowed_slot = b.ins().icmp_imm_s(IntCC::Equal, x, UB_TAG_SLOT);
    let slow_blk = b.create_block();

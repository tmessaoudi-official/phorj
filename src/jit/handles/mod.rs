...
pub(super) extern "C" fn rt_u_set_seal(ctx: *mut UbCtx, list: i64) -> i64 {
    let ctx = unsafe { &mut *ctx };
    ...
    if n >= 1 << 12 || ctx.bump + tslots > ctx.cap {
        // Orphaned on the VM-redo path (the VM re-materializes the list from bytecode), so free it
        // here — SYMMETRIC with the success-path `ctx.release(list)` below. For a flat list this is a
        // no-op; for a builder/slot-backed list it recycles the record instead of leaking it on this
        // rare (n≥4096 / arena-exhaustion) fallback. The earlier `list_values`-None return is the
        // analyzer-unreachable bad-handle case and is intentionally left untouched.
        ctx.release(list);
        return -1;
    }
    let base = ctx.bump as i64;
    ...
    ctx.bump += tslots;
    ctx.release(list);
    UB_TAG_FLAT_SET | (n << UB_MAP_CNT_SHIFT) | ((log2 as i64) << UB_MAP_LOG_SHIFT) | base
}

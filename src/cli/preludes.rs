...
pub(super) const CORE_MODULES: &[VirtualModule] = &[
    ...
    VirtualModule {
        ...
        bare_types: &["Attribute", "Entry", "Config", "EntryKind", "Deprecated"],
    },
    VirtualModule {
    ...
    // `Core.Log` (DEC-220 base; DEC-317 Log-v2): leveled logging with configurable channels.
    // Qualified calls `Log.<level>/channel/configure` resolve to the `Core.Log` natives; the prelude
    // declares the pure config/handle classes (`Level`, formatters, handlers, `ChannelConfig`,
    // `LogConfig`, `Channel`) over the `Core.Native.Log` emit kernel. Impure (stderr/file side
    // effects) ⇒ an importing program is quarantined from the byte-identity differential; the PHP
    // leg emits the same content contract via the gated `__phorj_log_*` helpers.
    VirtualModule {
        module: &["Core", "Log"],
        qualifier: "Log",
    ...
    // `Core.Native.Log` — the INTERNAL emit kernel the `Core.Log` prelude `Channel` calls.
    VirtualModule {

...
//!
//! IT RUNS PRE-EXPANSION, and every row depends on that. Both callers refuse before preludes are
//! injected (`cmd_transpile` on the `lex_parse` output, `transpile_program` before
//! `check_and_expand`), so a row keyed on a module the INJECTED preludes themselves import — as

//! Opaque native database resource handles (DEC-208 `Core.Database`, the enhanced-PDO primitive).
//!
...
//! implementors and the `Core.Database` module is not registered, so the variant is unconstructable.
//!
...
//! operand or interpolating it). Quarantined from the byte-identity differential (`Core.Database` natives
...
/// An opaque `Core.Database` resource handle carried by [`crate::value::Value::Db`]. Implementors are the
/// rusqlite-backed connection / statement types (feature-gated in `src/ext/database/natives.rs`). `as_any` lets a
/// `Core.Database` native downcast the type-erased handle back to its concrete type to perform the operation

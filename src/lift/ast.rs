...
pub fn php_item_name(item: &PhpItem) -> Option<&str> {
    match item {
        ...
        PhpItem::Interface(i) => Some(&i.name),
        PhpItem::Stmt(_) => None,
    }
}
...
pub enum PhpType {
    ...
    /// An array SHAPE, which is a phorj tuple — positional (DEC-288) or named-field (DEC-504).
    ///
    /// * positional (`array{int, string}`, or explicit ascending indices from zero,
    ///   `array{0: float, 1: float}`) carries `None`;
...
/// An interface: `interface Name [extends A, B] { public function m(…): T; }` (Lane R-5).
/// Methods are bodiless; constants are refused at the parser (phorj interfaces carry none).
#[derive(Debug, Clone, PartialEq)]

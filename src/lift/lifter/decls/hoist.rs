...
fn walk_expr(
    ...
    match e {
        ...
        // A destructure WRITES each binder and READS its initializer. Recorded as writes with no
        // literal, so a binder can never look like a hoistable constant.
        E::Destructure { binders, value } => {
            walk_expr(value, ctx, sightings, order);
            for b in binders {
                ctx.see(b, None, true, sightings, order);
            }
        }
        ...
        E::EmptyColl(_) => {}
        E::AppendSlot(inner) | E::Cast { value: inner, .. } | E::Declared { value: inner, .. } => {
            walk_expr(inner, ctx, sightings, order);
        }
        E::Index { base, index } => {

...
pub enum Item {
    ...
    /// item is accepted in TEST MODE — `phg test`, and (DEC-486) `phg check` / `check --json` / the LSP
    /// on any document that declares one — while `run`/`transpile`/`build` reject it as

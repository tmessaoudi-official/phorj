...
fn runnable_programs_keep_their_behavior() {
    ...
    let samples = [
        ...
        "package Main; import Core.Runtime.Entry; import Core.Runtime.EntryKind; import Core.Output;\n\
         ...
         #[Entry(kind: EntryKind.Cli)] function main(): void { Output.printLine(\"{add(2, 3)}\"); }",
        "package Main; import Core.Runtime.Entry; import Core.Runtime.EntryKind; import Core.Output;\n\
         enum Shape { Circle(int r), Square(int s) }\n\
         function area(Shape s): int { return match (s) { Circle(r) => r * r, Square(x) => x * x }; }\n\
         #[Entry(kind: EntryKind.Cli)] function main(): void { Output.printLine(\"{area(new Circle(3))}\"); }",
        "package Main; import Core.Runtime.Entry; import Core.Runtime.EntryKind; import Core.Output;\n\
         class Point { constructor(public int x, public int y) {} function sum(): int { return this.x + this.y; } }\n\
         #[Entry(kind: EntryKind.Cli)] function main(): void { Point p = new Point(2, 5); Output.printLine(\"{p.sum()}\"); }",
        "package Main; import Core.Runtime.Entry; import Core.Runtime.EntryKind; import Core.Output;\n\
         #[Entry(kind: EntryKind.Cli)] function main(): void { var dbl = function(int x): int => x * 2; Output.printLine(\"{3 |> dbl}\"); }",
        "package Main; import Core.Runtime.Entry; import Core.Runtime.EntryKind; import Core.Output;\n\
         #[Entry(kind: EntryKind.Cli)] function main(): void { int? m = null; Output.printLine(\"{m ?? -1}\"); for (int i in 0..3) { Output.printLine(\"{i}\"); } }",
    ];
    for s in samples {
...
fn comments_are_preserved() {
    let src = "package Main;\nimport Core.Runtime.Entry; import Core.Runtime.EntryKind;\n// a header comment\n#[Entry(kind: EntryKind.Cli)] function main(): void { /* body */ return; }\n";
    let out = fmt(src);
    assert!(
...
fn pipe_operator_survives_formatting() {
    let src = "package Main; import Core.Runtime.Entry; import Core.Runtime.EntryKind; import Core.Output;\n\
         function inc(int x): int { return x + 1; }\n\
         #[Entry(kind: EntryKind.Cli)] function main(): void { Output.printLine(\"{5 |> inc |> inc}\"); }";
    let out = fmt(src);
    assert!(

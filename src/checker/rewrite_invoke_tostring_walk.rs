//! The `#[Invoke]` / `#[ToString]` expression + statement walk (DEC-331 D9).
//!
//! Split out of `rewrite_invoke_tostring.rs` (CD-31): adding the `Item::Test` arm pushed that file
//! past Invariant 13's 500-line hard cap. The seam is the one the six DEC-356 splits already use —
//! the parent keeps the item-level traversal, the `_walk.rs` sibling keeps the expression and
//! statement recursion. Both matches stay exhaustive; neither may regrow a catch-all (the ratchet in
//! `ast::leaves` covers this file and its parent).

use super::Checker;
use crate::ast::{Expr, LambdaBody, MatchArm, MemberSep, Stmt, StrPart};
use crate::token::Span;
use std::collections::HashMap;

pub(super) type Names = HashMap<usize, String>;
...
pub(super) fn rexpr(e: Expr, inv: &Names, ts: &Names) -> Expr {
...
pub(super) fn rstmt(s: Stmt, inv: &Names, ts: &Names) -> Stmt {
    match s {
        Stmt::VarDecl {
            ty,
            name,
...
pub(super) fn rblock(stmts: Vec<Stmt>, inv: &Names, ts: &Names) -> Vec<Stmt> {

...
impl Checker {
    ...
    pub(in crate::checker) fn check_native_call(
        ...
        if n.params.iter().any(ty_has_param) || ty_has_param(&n.ret) {
            // Natives carry no ordered type-parameter list and take no turbofish (rejected upstream),
            // so type-params + turbofish are empty here — pure argument-driven inference (M-RT S7b).

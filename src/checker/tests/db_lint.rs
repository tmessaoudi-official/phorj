...
//! type-directed: it fires only on `Core.Database`'s `Connection.prepare(<interpolated SQL>)` when a hole splices a
...
//!
//! Like the `W-SECRET` tests these declare the `Connection`/`Statement` shape inline — `check()` never injects
//! the `Core.Database` prelude, and the lint keys on the class name `Connection` + the method `prepare` + an active
//! `Core.Database` import (the "nothing in the wind" gate), all of which a stub satisfies.

use super::support::*;

/// A minimal `Core.Database`-shaped stub: a `Connection` with `prepare` (the linted method) and a neighbouring
...
/// Build a program that imports `Core.Database` (satisfying the lint's import gate) and declares the stub.
fn with_import(body: &str) -> String {
    format!("import Core.Database; {DB_STUB} {body}")
}
...
fn without_core_db_import_a_user_db_class_never_warns() {
    // Import gate ("nothing in the wind"): a user class coincidentally named `Connection` with a `prepare`
    // method is NOT the Core.Database `Connection` — no import, no lint, even with an interpolated variable.
    let src = format!(
        "{DB_STUB} function main(): void {{ \
             Connection db = new Connection(\"x\"); \
             int id = 5; \
             Statement s = db.prepare(\"SELECT * FROM users WHERE id = {{id}}\"); \
         }}"
    );
    ...
    assert!(
        !warns_sql(&src),
        "unexpected W-SQL-INJECTION on a non-Core.Database class, got {:?}",
        warnings_of(&src)
    );
}
...
fn interpolation_into_a_non_prepare_method_never_warns() {
    // Type-directed to `prepare` specifically — a different `Connection` method with an interpolated argument

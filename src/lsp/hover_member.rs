...
pub(super) fn tuple_field_signature(
    ...
    let receiver = receiver_before(text, offset, false)?;
...
/// The identifier that precedes the `.` in front of the identifier under `offset` — or the `::`
/// when `colons` is set, the class-qualified spelling (`K::MAX`, row 5o); a tuple field has no `::`
/// form, so its caller passes `false`.
///
...
pub(super) fn receiver_before(text: &str, offset: usize, colons: bool) -> Option<String> {
    let b = text.as_bytes();
    ...
    let j = skip_ws(b, i);
    let sep = if j > 0 && b[j - 1] == b'.' {
        1
    } else if colons && j > 1 && b[j - 1] == b':' && b[j - 2] == b':' {
        2
    } else {
        return None;
    };
    ...
    let mut k = skip_ws(b, j - sep);
    if sep == 1 && k > 0 && b[k - 1] == b'?' {
        k = skip_ws(b, k - 1);
    }
    let end = k;

...
<!-- ═══════════════════════════════════════════════════════════════════════════════════
  ...
  De-containerized 2026-08-18: the sibling repo-local skills carrying the "container deltas" are
  deleted (global-is-reference ruling) — the surviving repo conventions live in `/phg-lenses`. The
  two operational rules that still bite here:
...
>
> ```
> /phg-qa-sweep — drive the SHIPPED phg binary and the non-cargo-test surfaces, and report what breaks.
>
> Usage: /phg-qa-sweep [--only <substr>] [--journey <name>] [--skip-build]
...
| 1 | `spine` | The three legs on the SHIPPED binary over `examples/**/*.phg`: `phg run` ≡ `phg run --tree-walker` ≡ `phg transpile` under the php RESOLVED by `scripts/toolchain.env` (globbed + bcmath-checked, never a pinned patch version) (**PHP 8.5** floor — never gate against the bare `php` on PATH, which is 8.6-dev and too permissive). Also `phg run --no-jit`, which must be byte-identical with no rebuild. This duplicates `tests/differential.rs` *deliberately*: it re-runs it through the installed artefact. |
...
| 8 | playground | SKIPPED | wasm-pack not installed |

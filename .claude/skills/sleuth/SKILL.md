...
## phorj lens K — MANDATORY additional agent (backend divergence)
...
**Hard stop** — **print as plain text and STOP until answered** (adaptation rule 1): question 'N findings (High: X | Medium: Y | Low: Z). Nothing has been changed — all findings above are proposals. What would you like to do?', options: 'Fix specific findings — list IDs in notes (e.g. A1, A3) (Recommended)' / 'Show all medium-confidence findings' / 'Show a specific category — specify in notes (e.g. category F)' / 'Nothing — close the report'.

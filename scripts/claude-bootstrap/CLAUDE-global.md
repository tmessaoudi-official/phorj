...
**Skills here are REPO-NATIVE** under `.claude/skills/` — read in place, nothing installed, no `~/.claude/refs/SKILLS.md` and no `~/.claude/skills/`. **`ls .claude/skills/` is the authoritative list — never restate a count in prose, it drifts** (this paragraph said "The 13 present" while a 14th was being added in the same change). As built (DEC-354 ruled 7 of the bundle's 48 IN; 5 predate it; `/forge` re-admitted by DEC-388.2; `/qa-sweep` added 2026-08-06): `/ask-human` (the plain-text question protocol), `/converge` (the DEC-268 ladder, mechanised), `/sweep` (Phase 6), `/expanding-context` (Phase 1/3C), `/sleuth` (behavioural bug hunt + backend-divergence lens K), `/inspect` (10-lens health), `/cross-check` (doc validation + `--drift`, doc-vs-code), `/aggregate-findings` (cross-stage dedup), `/gaps`, `/handoff`, `/pre-commit`, `/retrospective`, `/qa-sweep` (drives the real binary and the rendered surfaces), `/forge` (Chesterton's-Fence architecture review, `--quick` default). Anything else named in this framework (`/mega-analysis`, `/audit`, `/bootstrap`, `/templatize`, `/memory-*`, `/adapt-project`, `/skill-audit`, `/recent`, `/validate-infra` (native script instead — `scripts/validate-infra.sh`), `/loop` as a global) is **NOT installed here** — `/loop` is provided by the host instead.

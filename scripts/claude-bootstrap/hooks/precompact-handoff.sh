#!/usr/bin/env bash
# PreCompact hook (DEC-354) — write a handoff note BEFORE the context is compacted.
#
# Why this exists: context compaction loses working state. Invariant 19 already says only committed
...
#      with `jq`. Opt into an LLM narrative with PHORJ_HANDOFF_LLM=1.
...
HANDOFF_DIR="${PHORJ_HANDOFF_DIR:-$CWD/var/claude/handoff}"
...
# ── The project's own continuity pointers (Invariant 19) ───────────────────────────────────────
cursor_block() {
  local cursor="$CWD/docs/plans/SLICE-STATE.md"
  if [[ -f "$cursor" ]]; then
    printf -- '- Cursor: `docs/plans/SLICE-STATE.md` — read it FIRST after compaction.\n'
    printf -- '- Roadmap SSOT: `docs/plans/MASTER-PLAN.md` · Decisions: `docs/research/full-audit/raw/C-decisions.md`\n'
  else
    printf -- '- (no `docs/plans/SLICE-STATE.md` in this tree)\n'
  fi
}
...
if [[ "${PHORJ_HANDOFF_LLM:-0}" == "1" && -n "$TRANSCRIPT" && -f "$TRANSCRIPT" ]]; then
  if command -v claude >/dev/null 2>&1; then
    ...
    RAW=$(cd /tmp && env -u CLAUDE_PROJECT_DIR timeout 60 claude -p \
            --model "${PHORJ_HANDOFF_MODEL:-claude-haiku-4-5}" --max-turns 1 \
    ...
    log_obs WARN precompact-handoff "PHORJ_HANDOFF_LLM=1 but the claude CLI is not on PATH"
  fi
fi

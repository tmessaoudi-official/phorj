...
# (gitignored via `/var`), not ~/.claude/logs/. Changed 2026-08-06: the default was
# ~/.claude/logs/hooks-errors.log, which is wiped when the container is reclaimed, so every line a hook
# logged in a real session was written somewhere nobody could ever read — the observability rule was
# satisfied on paper and useless in practice. Nothing but the test harness sets OBS_LOG, so this
# default is what actually runs. (Ported from rent-watch, which hit the same dead end.)
